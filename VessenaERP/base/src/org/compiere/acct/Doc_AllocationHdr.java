...
public class Doc_AllocationHdr extends Doc
{
	...
	public ArrayList<Fact> createFacts (MAcctSchema as)
	{
		...
		// Cambios drasticos en la contabilizacion de las afectaciones.
	...
	/**
	 * OpenUp.	
	 * Descripcion : Creacion de asientos contables de afectaciones.
	 * @param as
	 * @return
	 * @author  Gabriel Vila 
	 * Fecha : 04/11/2010
	...
	private ArrayList<Fact> crearAsientosContables(MAcctSchema as) {
		...
		for (int i = 0; i < p_lines.length; i++)
		{
			...
			if (allocationSource.doubleValue()<0) allocationSource = allocationSource.multiply(new BigDecimal(-1));
			...
			if (invoice.isSOTrx())
			{
				MAccount acctinv =  MAccount.get (as.getCtx(), this.getValidCombination(invoice.getC_BPartner_ID(),
											   invoice.getC_Currency_ID(), true, Doc.ACCTTYPE_C_Receivable, as));
				FactLine fl; 
		
				MDocType doc = new MDocType(getCtx(), invoice.getC_DocType_ID(), getTrxName());
				
				if (doc.getDocBaseType().equalsIgnoreCase(DOCTYPE_ARCredit)){
					fact.createLine(null, acctinv, invoice.getC_Currency_ID(), allocationSource, null);
				}
				else{
					fact.createLine(null, acctinv, invoice.getC_Currency_ID(), null, allocationSource);	
				}

				if (payment!=null){
					MAccount acctpay =  MAccount.get (as.getCtx(), this.getValidCombination(payment.getC_BPartner_ID(),
							   payment.getC_Currency_ID(), true, Doc.ACCTTYPE_C_Receivable, as));
					if (doc.getDocBaseType().equalsIgnoreCase(DOCTYPE_ARCredit)){
						fl = fact.createLine(null, acctpay, payment.getC_Currency_ID(), null, allocationSource);
					}
					else{
						fl = fact.createLine(null, acctpay, payment.getC_Currency_ID(), allocationSource, null);	
					}
				}
			}
			else // Proveedores
			{
				MAccount acctinv =  MAccount.get (as.getCtx(), this.getValidCombination(invoice.getC_BPartner_ID(),
						   			invoice.getC_Currency_ID(), false, Doc.ACCTTYPE_V_Liability, as));

				FactLine fl; 
				
				MDocType doc = new MDocType(getCtx(), invoice.getC_DocType_ID(), getTrxName());

				if (doc.getDocBaseType().equalsIgnoreCase(DOCTYPE_APCredit)){
					fl = fact.createLine(null, acctinv, invoice.getC_Currency_ID(), null, allocationSource);
				}
				else{
					fl = fact.createLine(null, acctinv, invoice.getC_Currency_ID(), allocationSource, null);	
				}

				if (payment!=null){
					MAccount acctpay =  MAccount.get (as.getCtx(), this.getValidCombination(payment.getC_BPartner_ID(),
							payment.getC_Currency_ID(), false, Doc.ACCTTYPE_V_Liability, as));
					if (doc.getDocBaseType().equalsIgnoreCase(DOCTYPE_APCredit)){
						fl = fact.createLine(null, acctpay, payment.getC_Currency_ID(), allocationSource, null);
					}
					else{
	 ...
	 * Fecha : 04/11/2010
	 */
	private int getValidCombination(int keyID, int currencyID, boolean isReceipt, int acctType, MAcctSchema acctSchema){
		...
		try{
			...
			if (acctType==Doc.ACCTTYPE_UnallocatedCash){
