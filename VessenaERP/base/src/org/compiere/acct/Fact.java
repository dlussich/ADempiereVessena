...
public final class Fact
{
	...
	/***
	 * Creacion de linea de asiento contables recibiendo tipo de documento y numero.
	 * OpenUp Ltda. Issue #447 
	...
	public FactLine createLine (DocLine docLine, MAccount account,
			int C_Currency_ID, BigDecimal debitAmt, BigDecimal creditAmt,
			int cDocTypeID, String DocumentNo){
		
		FactLine value = this.createLine(docLine, account, C_Currency_ID, debitAmt, creditAmt);
		if (value != null){
			value.setC_DocType_ID(cDocTypeID);
			value.setDocumentNo(DocumentNo);
			value.saveEx();
		}
	...
	/**
	 * OpenUp.	
	 * Descripcion : Crea linea de asiento para redondeo. Bordes = 0.99, -0.99.
	 * @return
	 * @author  Gabriel Vila 
	 * Fecha : 22/02/2011
	 */
	public FactLine createRounding(int cDocTypeID, String DocumentNo, int cActivityID,
			int cActivityID1, int cActivityID2, int cActivityID3){
		
		/*
		if (m_doc.isMultiCurrency())
			return null;
		*/
		...
		if (diff.compareTo(bordeInferior)<0 || diff.compareTo(bordeSuperior)>0) return null;
		
		if (diff.compareTo(Env.ZERO) == 0) return null;
		...
		// Documento y centros de costo
		line.setC_DocType_ID(cDocTypeID);
		line.setDocumentNo(DocumentNo);
		if (cActivityID > 0) line.setC_Activity_ID(cActivityID);
		if (cActivityID1 > 0) line.setC_Activity_ID_1(cActivityID1);
		if (cActivityID2 > 0) line.setC_Activity_ID_2(cActivityID2);
		if (cActivityID3 > 0) line.setC_Activity_ID_3(cActivityID3);
