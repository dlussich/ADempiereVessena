...
public class MWFActivity extends X_AD_WF_Activity implements Runnable
{
	...
	private void setResponsible (MWFProcess process)
	{
		...
		// Envio nuevo parametro incluyendo el nodo asociado a esta actividad.
		// Comento y sustituyo codigo original.		
		...
		*/
		...
		if (AD_WF_Responsible_ID == 0){
			AD_WF_Responsible_ID = process.getAD_WF_Responsible_ID(this.getNode());
		}
		...
		else if (MWFNode.ACTION_DocumentAction.equals(action))
		{
			...
			if (m_po instanceof DocAction)
			{
				...
				try {
					...
					// Cuando quiero anular un documento de asiento diario no se porque me trae accion --
					// a pesar de que elijo la accion ANULAR. Trampa por ahora. 
					// 29/11/2012. LO COMENTE PORQUE SE SOLUCIONO QUITANDO AL ROL EL CHECK DE LA ACCION DE DOCUMENTO: PROCESAR
					/*
					if (m_po instanceof MJournal){
						if (doc.getDocStatus().equalsIgnoreCase(DocumentEngine.STATUS_Completed)){
							//m_node.setDocAction(DocumentEngine.ACTION_Void);
							//m_node.setDocAction(DocumentEngine.ACTION_ReActivate);
						}
					}
					*/
					// Fin OpenUp.
		...
		*/
		...
		else if (MWFNode.ACTION_UserChoice.equals(action))
		{
				...
				&& getPO(trx) instanceof DocAction)
			{
				...
				{
					//	Set Approver
					int startAD_User_ID = Env.getAD_User_ID(Env.getCtx());
				...
				else	//	fixed Approver
				{
					...
					if (resp.isHuman())
					{
						autoApproval = resp.getAD_User_ID() == Env.getAD_User_ID(Env.getCtx());
					...
					else if(resp.isRole())
					{
						...
						{
							if(urs[i].getAD_User_ID() == Env.getAD_User_ID(Env.getCtx()))
							{
								autoApproval = true;
								break;
							}
		...
		String textMsg) throws Exception
	{
		...
		*/
		...
		if (getNode().isUserApproval() && getPO(trx) instanceof DocAction)
		{
			...
			try
			{
				// OpenUp. Gabriel Vila. 05/12/2012. Issue #92.
				// Propiedades dinamicas de workflow
				if (m_po instanceof IDynamicWF) {
					((IDynamicWF)m_po).setApprovalInfo(this.getAD_WF_Responsible_ID(), textMsg);
				}
				// Fin OpenUp.
			...
			// OpenUp. Gabriel Vila. 11/12/2012. Issue #169
			// Al aprobar un documento se debe enviar nota y mail.
			if (!newState.equals(StateEngine.STATE_Aborted)) {
				...
				if (to.isNotificationEMail()) {
					MClient client = MClient.get(getCtx(), doc.getAD_Client_ID());
					client.sendEMail(doc.getDoc_User_ID(), "Aprobado "
							+ ": " + doc.getDocumentNo(), 
							(doc.getSummary() != null ? doc.getSummary() + "\n" : "" )
							+ (doc.getProcessMsg() != null ? doc.getProcessMsg() + "\n" : "") 
							+ (getTextMsg() != null ? getTextMsg() : ""), null);
				...
				// Send Note
				if (to.isNotificationNote()) {
					MNote note = new MNote(getCtx(), "Aprobado", doc.getDoc_User_ID(), null);
					note.setTextMsg((doc.getSummary() != null ? doc.getSummary() + "\n" : "" )
							+ (doc.getProcessMsg() != null ? doc.getProcessMsg() + "\n" : "") 
							+ (getTextMsg() != null ? getTextMsg() : ""));
					// 2007-06-08, matthiasO.
	...
	public String getSummary()
	{
		...
		// Agrego texto para numero de documento.
		// Comento y sustituyo.
		...
		*/
		...
		if (index != -1){
			sb.append(Msg.getElement(getCtx(), "DocumentNo")).append(" ");
			sb.append(po.get_Value(index)).append(" : ");
		}
		...
		if (sr != null)
		{
			...
			*/
			...
			if (user != null){
				sb.append(Msg.getElement(getCtx(), "AD_User_ID")).append(" ");
				sb.append(user.getName()).append(" ");
			}
