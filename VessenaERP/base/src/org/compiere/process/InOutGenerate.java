...
public class InOutGenerate extends SvrProcess
{
			...
			while (rs.next ())		//	Order
			{
				...
				if (completeOrder && MOrder.DELIVERYRULE_CompleteOrder.equals(order.getDeliveryRule()))
				{
					for (int i = 0; i < lines.length; i++)
					{
						...
						// Comento esta creacion de linea
						//createLine (order, line, toDeliver, storages, false);
						// Fin OpenUp.
					}
				}
		...
		MStorage[] storages, boolean force)
	{
		...
		// OpenUp. Gabriel Vila. 10/11/2010.
		// Cambio drastico, dejo solamente el codigo que habia dentro del if(storages==null) como correcto 
		// y por defecto. Comento todo lo demas porque me hacia mas de una linea por articulo
		// y esto me recontra jodia la factura.
		...
		//OpenUp. Nicolas Sarlabos. 19/12/2012.Seteo el descuento que trae la linea de la orden
		line.setFlatDiscount(orderLine.getFlatDiscount());
		//Fin OpenUp.
		if (!line.save())
