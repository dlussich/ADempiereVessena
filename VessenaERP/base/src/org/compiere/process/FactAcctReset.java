...
import org.openup.model.X_UY_ExchangeDiffHdr;
import org.openup.model.X_UY_MediosPago;
import org.openup.model.X_UY_MovBancariosHdr;
import org.openup.model.X_UY_YearEnd;
...
public class FactAcctReset extends SvrProcess
{
	...
	//		docBaseType = "= '" + MPeriodControl.DOCBASETYPE_GLDocument + "'";
		...
		//
		// OpenUp. Gabriel Vila. 17/11/2010.
		// Agrego Tablas propias por el tema del documenttype
		else if (AD_Table_ID == X_UY_MediosPago.Table_ID)
			docBaseType = "='CHQ' ";
		else if (AD_Table_ID == X_UY_MovBancariosHdr.Table_ID)
			docBaseType = " IN ('BCD','BIB','BGB','BDC','BDE','BDP','BDT','BTR','BPB','DCP','PVB','RCP','RCQ','RCT') ";
		else if (AD_Table_ID == X_UY_ExchangeDiffHdr.Table_ID)
			docBaseType = "='EXD' ";
		else if (AD_Table_ID == X_UY_YearEnd.Table_ID)
			docBaseType = "='YRE' ";
		// Fin OpenUp.
		...
		String sql1 = "UPDATE " + TableName
			...
			+ " AND DocStatus IN ('CO','CL') " // OpenUp. Gabriel Vila. 24/02/2012. Migracion. 			
