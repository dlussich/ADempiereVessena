...
public final class DisplayType
{
	...
	public static String getSQLDataType (int displayType, String columnName, int fieldLength)
	{
		...
		if (DisplayType.isID(displayType))
		{
			...
			// OpenUp. Gabriel Vila. 23/05/2011. Issue #634.
			// Para proceso de migracion de objetos del DD, encontre campos
			// ID que terminan en ID_To (ej: c_currency_id_to).
			// Estos deben pasar como numericos tambien.
			// ADEMAS LE PONGO TOLOWERCASE A TODAS LAS COMPARACIONES DE STRING EN EL IF !!!!
			else if (columnName.toLowerCase().endsWith("_id")
				|| columnName.toLowerCase().trim().endsWith("_id_to")
				|| columnName.toLowerCase().trim().endsWith("_id_from")
				|| columnName.toLowerCase().endsWith("tedby") 
				|| columnName.toLowerCase().endsWith("_acct") )
				return "NUMBER(10)";
