...
public class TimeUtil
{
	...
	/**
	 * Metodo que devuelve un nuevo Timestamp con la parte fecha de f2 y la parte hora de f1
	 * OpenUp Ltda.
	 * @author Nicolas Sarlabos - 16/05/2012
	 * @see
	 * @param f1
	 * @param f2
	 * @return
	...
	static public Timestamp copyDate(Timestamp date1,Timestamp date2){
		
		Timestamp newDate = null;
		
		if(date1 != null && date2 != null){

			//obtengo hora completa de "date1"
			...
			//asigno la hora a "date2"
			Calendar cal2 = Calendar.getInstance();
			cal2.setTime(date2);
			cal2.set(Calendar.HOUR_OF_DAY, hours);
			cal2.set(Calendar.MINUTE, minutes);
			cal2.set(Calendar.SECOND, seconds);
			cal2.set(Calendar.MILLISECOND, ms);
	...
	/**
	 * Metodo que suma o resta cantidad de dias manteniendo la hora original
	 * OpenUp Ltda. Issue #986 
	 * @author Nicolas Sarlabos - 24/05/2012
	 * @see
	 * @param date1
	 * @param date2
	 * @return
	...
	static public Timestamp addDaysWithHour (Timestamp day, int offset)
	{
		if (offset == 0)
		{
			return day;
		...
		cal2.set(Calendar.MILLISECOND, ms);
		if (offset == 0)
			return new Timestamp (cal2.getTimeInMillis());
		cal2.add(Calendar.DAY_OF_YEAR, offset);			//	may have a problem with negative (before 1/1)
		return new Timestamp (cal2.getTimeInMillis());
	}	
