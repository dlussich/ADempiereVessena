...
public class Login
{
	...
	//	s_log.info("loadWarehouses - Org: " + org.toStringX());
		...
		// OpenUp. Gabriel Vila. 14/10/2013. Issue #1409
		// Si debe cargar solo los almacenes asociados al usuario, lo hago.
		if (MSysConfig.getBooleanValue("UY_CHECK_WAREHOUSE_USER", false)){
			return this.getUserWarehouses(org);
		}
		// Fin OpenUp. Issue #1409
	...
	/***
	 * Obtiene y retorna solo los almacenes asociados al usuario logeado.
	 * OpenUp Ltda. Issue #1409 
	 * @author Gabriel Vila - 14/10/2013
	 * @see
	 * @param org
	 * @return
	 */
	public KeyNamePair[] getUserWarehouses (KeyNamePair org)
	{
		if (org == null)
			throw new IllegalArgumentException("Org missing");
		...
		ArrayList<KeyNamePair> list = new ArrayList<KeyNamePair>();
		KeyNamePair[] retValue = null;
		String sql = " SELECT wh.M_Warehouse_ID, wh.Name FROM M_Warehouse wh "
			//+ " WHERE wh.AD_Org_ID=? AND wh.IsActive='Y' "
			+ " WHERE wh.IsActive='Y' "				
			+ " AND wh."+ I_M_Warehouse.COLUMNNAME_IsInTransit+"='N' "
			+ " AND wh.m_warehouse_id in " 
			+ " (select m_warehouse_id from uy_warehouseuser "
			+ " where ad_user_id =" + Env.getAD_User_ID(m_ctx) + ") "
			+ " ORDER BY wh.Name ";
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try
		{
			...
			if (!rs.next())
			{
				rs.close();
				pstmt.close();
				log.info("No Warehouses for Org: " + org.toStringX() + " and User: " + Env.getAD_User_ID(m_ctx));
				return null;
			}
