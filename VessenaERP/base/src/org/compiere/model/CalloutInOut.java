...
public class CalloutInOut extends CalloutEngine
{
	...
	public String order (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		if (order.get_ID() != 0)
		{
			...
			// No seteo campos con IDS = 0. Pongo ifs en varias partes.
	...
	public String bpartner (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		try
		{
			...
			if (rs.next())
			{
				...
				// OpenUp. Gabriel Vila. 30/06/2014. Issue #2343
				// Para ventas tambien seteo el partner_location
				else{
					Integer ii = new Integer(rs.getInt("C_BPartner_Location_ID"));
					if (rs.wasNull())
						mTab.setValue("C_BPartner_Location_ID", null);
				...
				// Fin OpenUp. Issue #2343.	

				
				//OpenUp. Nicolas Sarlabos. 08/11/2012. Comento la validacion de credito disponible
	...
	public String product (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		// OpenUp Ltda - #5544 - Raul Capecce - 29/02/2016
		// Se carga en la ventana el impuesto asociado al producto
		tax(product, ctx, WindowNo, mTab, mField, value);
		// Fin #5544
		...
		if (IsSOTrx)
		{			
			//OpenUp Gabriel Vila 25/05/2012 issue #1000
			mTab.setValue("C_UOM_ID", 100); //CUIDADO, PUEDE TRAER PROBLEMAS
			mTab.setValue("UY_ProdName",product.getName());
			//fin OpenUp Gabriel Vila 25/05/2012 issue #1000
			return "";
		}
	...
	/**
	 * OpenUp Ltda - #5544 - Raul Capecce - 29/02/2016
	 * Carga el tipo de impuesto del producto
	 * @param mProduct
	 * @param ctx context
	 * @param WindowNo window no
	 * @param mTab tab model
	 ...
	 * @return error message or ""
	 */
	public String tax (MProduct mProduct, Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value) {
		
		MDocType doc = new MDocType(ctx, Env.getContextAsInt(ctx, WindowNo, WindowNo,"C_DocType_ID"), null);
		if (doc.getValue().equalsIgnoreCase("remitocli")) {
			MTaxCategory taxC = (MTaxCategory) mProduct.getC_TaxCategory();
			MTax tax = taxC.getDefaultTax();
	...
	/***
	 * Setea el estado de stock segun el tipo/motivo de devolucion
	 ...
	 * @return
	 */
	public String setStockStatus(Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value){
		
		if (value == null) return "";
		
		int inOutType_id = (Integer)value;
		if (inOutType_id <= 0) return "";
		
		MStockStatus status = null;
		MInOutType type = new MInOutType(ctx, inOutType_id, null);
		
		if(!type.getValue().equalsIgnoreCase("")){
			if(type.getValue().equalsIgnoreCase("bien")){
				
				//obtengo estado de stock APROBADO
				status = MStockStatus.forValue(ctx, "aprobado", null);
				
				//seteo estado de stock APROBADO
				mTab.setValue(X_M_InOut.COLUMNNAME_UY_StockStatus_ID, status.get_ID());				
				
			} else {
				
				//obtengo estado de stock RESIDUAL
				status = MStockStatus.forValue(ctx, "cuarentena", null);
				
				//seteo estado de stock RESIDUAL
				mTab.setValue(X_M_InOut.COLUMNNAME_UY_StockStatus_ID, status.get_ID());	
	...
	/***
	 * Setea el almacen segun el motivo de devolucion
	 * OpenUp Ltda. #1206
	 * @author Nicolas Sarlabos - 13/09/2013
	 * @see
	 * @param ctx
	 * @param WindowNo
	 * @param mTab
	...
	public String inOutType(Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value){
		...
		if(typeID==1000000) mTab.setValue(X_M_InOut.COLUMNNAME_M_Warehouse_ID, 1000004);
		if(typeID==1000001) mTab.setValue(X_M_InOut.COLUMNNAME_M_Warehouse_ID, 1000017);
