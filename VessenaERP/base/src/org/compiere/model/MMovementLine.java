...
public class MMovementLine extends X_M_MovementLine
{
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		// OpenUp. Gabriel Vila. 05/07/2011. Issue #719.
		// Cambio el if de la validacion para que ahora incluya el estado 
		...
		if (getM_Locator_ID() == getM_LocatorTo_ID() 
				&& getM_AttributeSetInstance_ID() == getM_AttributeSetInstanceTo_ID()
				&& getUY_StockStatus_ID() == getUY_StockStatus_To_ID())
		{
			log.saveError("Error", "No es posible mover a la misma ubicacion con igual estado de stock.");
			return false;
		}
		// Fin OpenUp. Issue #719.
		...
		//OpenUp. Nicolas Sarlabos. 30/08/2013. #1254
		MMovement hdr = new MMovement(getCtx(),this.getM_Movement_ID(),get_TrxName());
		MDocType doc = new MDocType(getCtx(),hdr.getC_DocType_ID(),get_TrxName());
		
		if(doc.getValue()!=null){
			if(doc.getValue().equalsIgnoreCase("transfablog")){
				
				MStockStatus status = MStockStatus.forValue(Env.getCtx(), "aprobado", null); 
				
				if(status.get_ID()>0){ //el estado de stock siempre debe ser APROBADO
					
					this.setUY_StockStatus_ID(status.get_ID());
					this.setUY_StockStatus_To_ID(status.get_ID());					
				}
				
				//me aseguro de setear el origen FABRICA y destino LOGISTICA para este documento
				this.setM_Locator_ID(1000002);
				this.setM_LocatorTo_ID(1000012);
