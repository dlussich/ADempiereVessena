...
public class ProductCost
{
	...
	// Cuentas para transporte
	public static final int ACCTTYPE_P_TR_FleteTransito = 24;
	public static final int ACCTTYPE_P_TR_FleteImpNacional = 25;
	public static final int ACCTTYPE_P_TR_FleteExpNacional = 26;
	...
	public MAccount getAccount(int AcctType, MAcctSchema as)
	{
		...
		String sql = "SELECT P_Revenue_Acct, P_Expense_Acct, P_Asset_Acct, P_Cogs_Acct, "	//	1..4
			...
			// OpenUp. Gabriel Vila. 25/09/2014. Issue #1405
			// Me traigo cuentas del modulo de transporte
			+ ", P_TR_FleteTransito_Acct, P_TR_FleteImpNacional_Acct, P_TR_FleteExpNacional_Acct "	// 24,25,26
			// Fin OpenUp. Issue #1405
		...
		// OpenUp. Gabriel Vila. 01/11/2012. Issue #83.
		// Si el producto no tiene contabilidad creada utilizo la contabilidad		
		// de su categoria.
		...
		*/
		if (validCombination_ID == 0){
			MProduct prod = new MProduct(Env.getCtx(), m_M_Product_ID, null);
			if (prod.get_ID() > 0){
				if (prod.getM_Product_Category_ID() > 0){
					MProductCategory cat = new MProductCategory(Env.getCtx(), prod.getM_Product_Category_ID(), null);
					MProductCategoryAcct catacct = cat.getCategoryAccounting();
					if (AcctType == ACCTTYPE_P_InventoryClearing){
						validCombination_ID = catacct.getP_InventoryClearing_Acct();
					}
					else if (AcctType == ACCTTYPE_P_Revenue){
						validCombination_ID = catacct.getP_Revenue_Acct();
					}
				}
			}
		}
