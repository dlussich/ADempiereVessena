...
public class MSequence extends X_AD_Sequence
{
	...
	/***
	 * Obtiene secuencial por empresa y nombre (no por tabla).
	 ...
	 * @return
	 */
	public static MSequence get (Properties ctx, int adClientID, String SequenceName, String trxName)
	{
		MSequence retValue = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		...
		try
		{
			...
			if (rs.next ())
				retValue = new MSequence (ctx, rs.getInt(1), trxName);

			if (rs.next())
				s_log.log(Level.SEVERE, "More then one sequence for Name " + SequenceName);
		}
		catch (Exception e)
		{
	...
	/***
	 * Obtiene ID de secuencial segun nombre y bloquea el registro para poder luego ser incrementado.
	 * OpenUp Ltda. Issue #1405
	 * @author Gabriel Vila - 10/01/2015
	 * @see
	 * @param AD_Client_ID
	 ...
	 * @return
	 */
	public static int getNextID_ForName (int AD_Client_ID, String SequenceName, String trxName)
	{
		int retValue = -1;
		...
		String selectSQL = "SELECT CurrentNext, CurrentNextSys, IncrementNo, AD_Sequence_ID "
						...
						+ " WHERE AD_Client_ID=? " 
						+ " AND UPPER(Name)=? "
						+ " AND IsActive='Y' AND IsAutoSequence='Y' "
						+ " FOR UPDATE OF AD_Sequence " ;//OF AD_Sequence "; POSTGRES WORKS, FOR MYSQL SYNTAX						
		...
		try
		{
			...
			if (rs.next())
			{
				...
				int AD_Sequence_ID = rs.getInt(4);
				DB.executeUpdateEx("UPDATE AD_Sequence SET CurrentNext = CurrentNext + incrementno WHERE AD_Sequence_ID =" + AD_Sequence_ID, trxName);
				
			}
			else{
				s_log.severe ("No record found for Name - " + SequenceName);
			}
		}
		catch (Exception e)
		{
