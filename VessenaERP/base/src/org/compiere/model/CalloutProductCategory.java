...
public class CalloutProductCategory extends CalloutEngine
{
	...
	/**
	 * OpenUp Nicolas Garcia 13/06/2011 Issue #707.
	 * Unidad de medida por defecto.
	 */
	public String setLocator(Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value, Object oldValue){

		if (isCalloutActive() || value == null)
			return "";

		int productCategoryID = (Integer) mTab.getValue(MProduct.COLUMNNAME_M_Product_Category_ID);
		MProductCategory model = new MProductCategory(ctx, productCategoryID, null);
		//La unidad de medida por defecto de la categoria se la coloco a la unidad de medida por defecto.
		mTab.setValue(MProduct.COLUMNNAME_C_UOM_ID, model.getC_UOM_ID());
		mTab.setValue(MProduct.COLUMNNAME_M_Locator_ID, model.getM_Locator_ID());
