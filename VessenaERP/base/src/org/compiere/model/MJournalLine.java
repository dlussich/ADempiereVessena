...
public class MJournalLine extends X_GL_JournalLine
{
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		// OpenUp. Gabriel Vila. 11/09/2012. Issue #20.
		// Ahora cargo la cuenta y luego Seteo Combinacion segun esa cuenta contable
		if (this.getC_ElementValue_ID() > 0){
			MAccount acct = MAccount.forElementValue(getCtx(), this.getC_ElementValue_ID(), null);
			this.setC_ValidCombination_ID(acct);
			
			
			// OpenUp. Gabriel Vila. 23/01/2013. Issue #249
			// Segun parametrizacion valido centros de costos obligatorios.
			if (MSysConfig.getBooleanValue("UY_JOURNAL_CC_NOT_NULL", false, this.getAD_Client_ID())){
				MElementValue ev = (MElementValue)this.getC_ElementValue();
				if (ev.getAccountType().equalsIgnoreCase(X_C_ElementValue.ACCOUNTTYPE_Expense)){
					if (this.getC_Activity_ID_1() <= 0){
						throw new AdempiereException("Debe ingresar centro de costo para cuenta : " + ev.getName());
					}
				}
				else if (ev.getAccountType().equalsIgnoreCase(X_C_ElementValue.ACCOUNTTYPE_Revenue)){
					if ( (this.getC_Activity_ID_1() <= 0) || (this.getC_Activity_ID_2() <= 0) 
						 || (this.getC_Activity_ID_3() <= 0)){
						throw new AdempiereException("Debe ingresar centro de costo para cuenta : " + ev.getName());
					}
				}
			}
		...
		if (newRecord 
			...
			|| is_ValueChanged("C_ElementValue_ID")  // OpenUp. Gabriel Vila. 11/09/2012. Issue #20
		...
		// OpenUp. Gabriel Vila. 23/11/2012. Issue #29.
		// Me aseguro que las lineas tengan bien la fecha de contabilizacion y la tasa de cambio
		this.setDateAcct(getParent().getDateAcct());
		// Fin OpenUp
	...
	private boolean updateJournalTotal()
	{
		...
		MJournal journal = new MJournal(getCtx(), this.getGL_Journal_ID(), get_TrxName());
		MDocType doc = new MDocType(getCtx(), journal.getC_DocType_ID(), null);
