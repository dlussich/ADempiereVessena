	...
	implements Serializable, Comparator, Evaluatee, Cloneable
{
	...
	// Atributos para manejo de ventana
	boolean refreshWindow = false;
	// Fin issue #906	

	// OpenUp. Gabriel Vila. 30/08/2013. Issue #1263.
	// Asociacion de GridTable con PO
	private GridTable tableModel = null;
	...
	*/ 
	...
	//	int index = -1;
		...
		try
		{
			...
			{
				// OpenUp. Gabriel Vila. 25/05/2011. Issue #678.
				// En el proceso de migracion cuando no hay un po con un determinado ID lo cual es
				// totalmente posible, me llena los logs con este mensaje.
				// Lo quito ya que no me sirve de nada que lo tire como error.
	...
	protected boolean load (ResultSet rs)
	{
		...
		for (index = 0; index < size; index++)
		{
			...
			// OpenUp. Gabriel Vila. 25/06/2012.
			// Este arreglo de la gente de Adempiere hace que no pueda leer
			// los valores de las columnas virtuales. Lo comento.
			...
			*/
			...
			{
				// OpenUp. Gabriel Vila. 26/06/2012. Comento printstacktrace porque no tiene sentido.
...
//				int index = p_info.getColumnIndex(column);
	 ...
	 * 	@param AD_Client_ID client
	 *  
	 *  OpenUp. Gabriel Vila. 06/11/2012. A veces necesito setear la empresa.
	 */
...
//		int dt = p_info.getColumnDisplayType(index);
	...
	private boolean insertTranslations()
	{
		...
		// OpenUp. Gabriel Vila.
		// Cuando estoy dando de alta un Field en una Tab, tomo nombre, descripcion y help desde Elemento
		if (this instanceof MField){
			String action = " insert into ad_field_trl (ad_language, ad_field_id, ad_client_id, ad_org_id, " +
					"isactive, created, createdby, updated, updatedby, name, description, help, istranslated) ";
			String sql = " select trl.ad_language, ff.ad_field_id, 0, 0, 'Y', now(), 100, now(), 100, " +
					     " trl.name, trl.description, trl.help, 'Y' " +
					     " from ad_field ff " +
					     " inner join ad_column cc on ff.ad_column_id = cc.ad_column_id " +
					     " inner join ad_element_trl trl on cc.ad_element_id = trl.ad_element_id " +
					     " where ff.ad_field_id =" + this.get_ID();

			int no = DB.executeUpdate(action + sql, m_trxName);
			log.fine("#" + no);
			
			// Puede pasar que el elemento no tenga traduccion. 
			// Si es asi, sigo como lo hacia antes.
			if( no > 0){
				return true;
			}
		}
...
//				int index = p_info.getColumnIndex(columnName);
	 ...
	 * 
	 * OpenUp.	
	 * Descripcion :Metodo que copia toda la informacion de un PO a otro.
	 * @param from
	 * @param to
	 * @author  Nicolas Garcia
	 * Fecha : 27/10/2011
	 */
	public static void devolverCopia(PO from, PO to) {

		{
			for (int i = 0; i < from.m_newValues.length; i++) {
				if (from.p_info.isVirtualColumn(i) || from.p_info.isKey(i)) // KeyColumn
					continue;
				String colName = from.p_info.getColumnName(i);
				// Ignore Standard Values
				if (colName.startsWith("Created") || colName.startsWith("Updated") || colName.equals("IsActive") || colName.equals("AD_Client_ID")
						|| colName.equals("AD_Org_ID") || colName.equals("Processing"))
				; // ignore
				else
	 ...
	 */
	public void setTableModel(GridTable tableModel) {
