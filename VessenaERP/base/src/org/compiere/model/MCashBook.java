...
public class MCashBook extends X_C_CashBook
{
	...
	public static MCashBook get (Properties ctx, int AD_Org_ID, int C_Currency_ID)
	{
		...
		//	Get from DB
		whereClause = I_C_CashBook.COLUMNNAME_AD_Org_ID+"=? AND "+I_C_CashBook.COLUMNNAME_C_Currency_ID+"=?";
		retValue = new Query(ctx,I_C_CashBook.Table_Name,whereClause,null)
		...
		if (retValue!=null)
			...
			//OpenUp. Nicolas Sarlabos. 16/01/2013. Obtengo para cualquier organizacion
			whereClause = I_C_CashBook.COLUMNNAME_C_Currency_ID+"=?";
			retValue = new Query(ctx,I_C_CashBook.Table_Name,whereClause,null)
