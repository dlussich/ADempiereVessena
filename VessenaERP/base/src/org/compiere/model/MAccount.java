...
public class MAccount extends X_C_ValidCombination
{
		...
		int User1_ID, int User2_ID, int UserElement1_ID, int UserElement2_ID)
	{
		...
		// Llamo a nuevo metodo con cambio de firma, ahora recibo transaccion.
		return MAccount.get(ctx, AD_Client_ID, AD_Org_ID, C_AcctSchema_ID, Account_ID, C_SubAcct_ID, M_Product_ID, 
				C_BPartner_ID, AD_OrgTrx_ID, C_LocFrom_ID, C_LocTo_ID, C_SalesRegion_ID, C_Project_ID, C_Campaign_ID, 
				C_Activity_ID, User1_ID, User2_ID, UserElement1_ID, UserElement2_ID, null);
	...
	/**
	 * Cambio firma al metodo original para que reciba una transaccion.
	 * OpenUp Ltda. Issue # 
	 * @author Hp - 08/06/2012
	 * @see
	 * @param ctx
	 * @param AD_Client_ID
	 * @param AD_Org_ID
	...
	public void setValueDescription()
	{
		...
		{
			// OpenUp. Gabriel Vila. 08/06/2012. 
			// Comento codigo por cambios en armado de descripcion de combinaciones
			/*
			if (i > 0)
			{
...
`			*/
			...
			else if (MAcctSchemaElement.ELEMENTTYPE_Account.equals(element.getElementType()))
			{
				if (getAccount_ID() != 0)
				{
					...
					*/
					String alias = m_accountEV.getAlias();
					// Actualizo alias de la combinacion segun el alias de la cuenta
					if (alias != null){
						this.setAlias(alias);
					}
					else{
					...
					combiStr = (alias != null) ? alias : m_accountEV.getValue();
					combiStr += "_" + m_accountEV.getName();
					descrStr = m_accountEV.getName();
					// Fin OpenUp
	...
	/***
	 * Obtiene y retorna combinacion segun id recibido de cuenta contable.
	 * OpenUp Ltda. Issue #20 
	 * @author Gabriel Vila - 11/09/2012
	 * @see
	 * @param ctx
	 ...
	 * @return
	 */
	public static MAccount forElementValue(Properties ctx, int cElementValueID, String trxName){
		
		String whereClause = X_C_ValidCombination.COLUMNNAME_Account_ID + "=" + cElementValueID;
		
		MAccount acct = new Query(ctx, I_C_ValidCombination.Table_Name, whereClause, trxName)
		.first();
