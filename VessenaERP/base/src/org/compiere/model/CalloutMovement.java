...
public class CalloutMovement extends CalloutEngine
{
	...
	/***
	 * Setea campos necesarios para movimientos de inventario Fabrica - Logistica y viceversa.
	 * OpenUp #1254
	 * @author Nicolas Sarlabos - 30/08/2013
	 * @see
	 * @return
	 */	
	public String setForTransference(Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value) {
		if (value == null)
			return "";
		
		int docID = Env.getContextAsInt(ctx, WindowNo, "C_DocType_ID");
		MDocType doc = new MDocType(Env.getCtx(),docID,null);
		...
		if(doc.getValue()!=null){
			if(doc.getValue().equalsIgnoreCase("transfablog")){
				...
				if(status.get_ID()>0){
					...
					mTab.setValue("UY_StockStatus_ID", status.get_ID());
					mTab.setValue("UY_StockStatus_To_ID", status.get_ID());

				}

				mTab.setValue("M_Locator_ID", 1000002);
				mTab.setValue("M_LocatorTo_ID", 1000012);

			} else if (doc.getValue().equalsIgnoreCase("translogfab")) {
				...
				mTab.setValue("M_Locator_ID", 1000012);
				mTab.setValue("M_LocatorTo_ID", 1000002);
