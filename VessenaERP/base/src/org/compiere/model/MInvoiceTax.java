...
public class MInvoiceTax extends X_C_InvoiceTax
{
	...
	public boolean calculateTaxFromLines ()
	{
		...
		sql = "SELECT il.LineNetAmt, COALESCE(il.TaxAmt,0), i.IsSOTrx "
				+ "FROM C_InvoiceLine il"
				+ " INNER JOIN C_Invoice i ON (il.C_Invoice_ID=i.C_Invoice_ID) "
				+ "WHERE il.C_Invoice_ID=? AND il.C_Tax_ID=?";
		
		//Openup. Nicolas Sarlabos. 27/02/2014. #1623.
		MInvoice inv = new MInvoice(getCtx(),getC_Invoice_ID(),get_TrxName()); //instancio cabezal de documento
		MDocType doc = new MDocType(getCtx(),inv.getC_DocTypeTarget_ID(),get_TrxName()); //instancio tipo de documento
		
		//solo para documento "Proforma Expediente" y "Factura Flete"
		if(doc.getValue()!=null && (doc.getValue().equalsIgnoreCase("fileprofinvoice") || doc.getValue().equalsIgnoreCase("freightinvoice")
				|| doc.getValue().equalsIgnoreCase("et-ticket") || doc.getValue().equalsIgnoreCase("et-nc") || doc.getValue().equalsIgnoreCase("customerncflete"))){
			...
			if((trip.getTripType().equalsIgnoreCase("IMPORTACION") || trip.getTripType().equalsIgnoreCase("NACIONAL")) && !trip.isInTransit()){ 
				...
				sql = "SELECT coalesce(il.nationalamt,0), COALESCE(il.TaxAmt,0), i.IsSOTrx "
						+ "FROM C_InvoiceLine il"
						+ " INNER JOIN C_Invoice i ON (il.C_Invoice_ID=i.C_Invoice_ID) "
						+ "WHERE il.C_Invoice_ID=? AND il.C_Tax_ID=?";				
			}			
