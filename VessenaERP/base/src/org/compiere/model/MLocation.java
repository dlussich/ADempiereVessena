...
public class MLocation extends X_C_Location implements Comparator
{
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		// OpenUp. Gabriel Vila. 24/02/2012. Issue #migracion
		// Comento lineas originales.
		/*
		...
		*/
		...
		// OpenUp. Nicolas Garcia. 30/08/2011. Issue #759.
		// Por motivos practicos se resuelve duplicar la informacion

		// Departamento
		if (this.getUY_Departamentos_ID() > 0) {
			MDepartamentos dep = new MDepartamentos(getCtx(), this.getUY_Departamentos_ID(), null);
			this.setCity(dep.getName());
		} else {
			this.setCity("NO Departamento");
		}

		// Localidad
		if (this.getUY_Localidades_ID() > 0) {
			MLocalidades loc = new MLocalidades(getCtx(), getUY_Localidades_ID(), null);
			this.setAddress4(loc.getName());
			// Seteo el codigo postal
			this.setPostal(loc.getzipcode());
		} else {
			this.setAddress4("NO Localidad");
		}
		// Fin Issue #issue
