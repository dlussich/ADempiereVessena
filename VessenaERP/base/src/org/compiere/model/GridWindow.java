...
public class GridWindow implements Serializable
{
	...
	/**
	 * Se crea metodo para realizar loa pregunta
	 * OpenUp Ltda. Issue #817 
	 * @author Gabriel Vila - 16/02/2012
	 * @see
	 * @return
	 */
	public boolean disposeOpen() {
		boolean salida = true;
		GridTab campos = this.getTab(0);
		GridField docStatus = null; // getField("DocStatus");
		if (campos != null) {
			docStatus = campos.getField("DocStatus");
			if (docStatus != null){
				if (docStatus.getValue() == null) {
					docStatus = null;
				}
			}
		}
		if (docStatus != null) {
			if (docStatus.getValue().toString().equalsIgnoreCase("IP")
					|| docStatus.getValue().toString().equalsIgnoreCase("DR")
					|| docStatus.getValue().toString().equalsIgnoreCase("IN")) {
				if (!ADialog.ask(getWindowNo(), null, null,
						"Confirma salir sin realizar Accion sobre el Documento ?")) {
					salida = false;
				}
			}
		}
