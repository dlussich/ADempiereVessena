...
public class CalloutInventory extends CalloutEngine
{
	...
	public String product (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		// OpenUp. Gabriel Vila. 26/04/2012. Issue #1010.
		// Comento lineas que dan problema ya que sacan stock de la storage
		...
		*/
		...
		// OpenUp. Gabriel Vila. 26/04/2012. Issue #1010.
		// Necesito considerar el estado de stock para obtener la cantidad en libros
		int uyStockStatus = 0;
		Integer StockStatus = (Integer)mTab.getValue("UY_StockStatus_ID");
		if (StockStatus != null)
			uyStockStatus = StockStatus.intValue();
		if (uyStockStatus == 0)
			return "";
		// Fin OpenUp.
		...
		try {
			...
			// Obtengo stock desde modelo de OpenUp.
			MInventory header = new MInventory(ctx, (Integer)mTab.getValue(X_M_Inventory.COLUMNNAME_M_Inventory_ID), null); 
			//OpenUp Nicolas Sarlabos 26/04/2012 #1010,paso como parametro el uy_stockstatus_id
			bd = setQtyBookOpenUp(M_AttributeSetInstance_ID, M_Product_ID, M_Locator_ID, header.getMovementDate(), uyStockStatus);
	...
	/**
	 * Obtiene cantidad en libros desde modelo de stock de OpenUp.
	 * OpenUp Ltda. Issue # 719 
	 ...
	 * @throws Exception
	 */
	private BigDecimal setQtyBookOpenUp (int M_AttributeSetInstance_ID, int M_Product_ID, 
									     int M_Locator_ID, Timestamp movementDate, int uyStockStatusID) throws Exception {
		// Set QtyBook from first storage location
		BigDecimal bd = null;
		String sql = "";

		//OpenUp Nicolas Sarlabos 26/04/2012
		sql = " select sum(qty) from stk_sts_prodxlocxasixday  "
				+ " WHERE M_Product_ID=?"	//	1
				+ " AND M_Locator_ID=?"
				+ " AND UY_StockStatus_ID=?"				
				+ " AND M_AttributeSetInstance_ID=? "
				+ " AND MovementDate <= '" + movementDate +"'" ;	//OpenUp M.R. 24-05-2011 issue #414	Agrego sintaxis sql para que compare el inventario con la fecha del dia del cabezal del inventario
			if (M_AttributeSetInstance_ID == 0)
				sql = " select sum(qty) from stk_sts_prodxlocxasixday  "
				+ "WHERE M_Product_ID=?"	//	1
				+ " AND M_Locator_ID=?"
		...
		//Fin OpenUp Nicolas Sarlabos 26/04/2012
		try
		{
			PreparedStatement pstmt = DB.prepareStatement(sql, null);
			pstmt.setInt(1, M_Product_ID);
			pstmt.setInt(2, M_Locator_ID);
			pstmt.setInt(3, uyStockStatusID);
			if (M_AttributeSetInstance_ID != 0)
