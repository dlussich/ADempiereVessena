...
public class MJournal extends X_GL_Journal implements DocAction
{
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		// OpenUp. Gabriel Vila. 21/01/2013. Issue #259
		// Seteo bien el periodo 
		if (getDateAcct() != null){
			setC_Period_ID(MPeriod.getC_Period_ID(getCtx(), getDateAcct(), getAD_Org_ID()));
		}
		// Fin OpenUp.
	...
	{
		// OpenUp. Gabriel Vila. 08/09/2012. Issue #20.
		// Si este asiento diario es Simple, no debo actualizar lote.
		MDocType doc = new MDocType(getCtx(), this.getC_DocType_ID(), null);
		if (doc.getDocSubTypeSO() != null){
			if (doc.getDocSubTypeSO().equalsIgnoreCase("SJ")){
				return true;
			}
		}		
		// Fin OpenUp
	...
	public String prepareIt()
	{
		...
		// OpenUp. Gabriel Vila. 18/09/2012. Issue #29.
		// Comento validacion de periodo estandar para asiento diario.
		...
		*/
		...
		for (int i = 0; i < lines.length; i++)
		{
			...
			// Comento y cambio sumarizacion a campos en moneda nacional ya que asi sumaba
			// pesos con dolares.
		...
		if (AmtSourceDr.compareTo(AmtSourceCr) != 0)
		{
			...
			{
				// OpenUp. Gabriel Vila. 18/09/2012. Issue #29. 
				// Como voy a manejar cuenta de redondeo en asientos, no controlo por ahora el descuadre.
				/*
				m_processMsg = "Asiento Desbalanceado : " + " Debitos =" + AmtSourceDr + " - Creditos =" + AmtSourceCr + " - Fecha =" + this.getDateAcct().toString();
				return DocAction.STATUS_Invalid;
				*/
				// Fin OpenUp
			}
	...
	public String completeIt()
	{
		...
		//OpenUp. Nicolas Sarlabos. 19/07/2013. #1140
		if(!this.isBalanced()) throw new AdempiereException("Imposible completar: asiento NO balanceado, verifique lineas del documento");
		//Fin OpenUp #1140
		...
		// OpenUp. Gabriel Vila. 10/09/2012. Issue #20
		// Cambio docaction 
	...
	/**
	 * Valida que las lineas del asiento esten balanceadas antes de completar el documento.
	 * OpenUp Ltda. Issue #1140
	 * @author Nicolas Sarlabos - 19/07/2013
	 * @see 
	 * @return
	 */
	private boolean isBalanced() {
		...
		BigDecimal bordeInferior = new BigDecimal(-0.50);
		BigDecimal bordeSuperior = new BigDecimal(0.50);
		MJournalLine [] jLines = this.getLines(false);

		for (int i = 0; i < jLines.length; i++){

			totalDR = totalDR.add(jLines[i].getAmtAcctDr());
			totalCR = totalCR.add(jLines[i].getAmtAcctCr());			
	...
	public boolean voidIt()
	{
		...
		if (DOCSTATUS_Drafted.equals(getDocStatus()) 
			...
			|| DOCSTATUS_Completed.equals(getDocStatus()))  // OpenUp. Gabriel Vila. 15/11/2012. Issue #29. Agrego estado completo
		{
			setProcessed(true);
			...
			FactLine.deleteFact(I_GL_Journal.Table_ID, this.get_ID(), get_TrxName());
	...
	public MJournal reverseAccrualIt (int GL_JournalBatch_ID)
	{
		...
		MJournal reverse = new MJournal (this);
		reverse.setGL_JournalBatch_ID(GL_JournalBatch_ID);
		//reverse.setDateDoc(new Timestamp(System.currentTimeMillis()));OpenUp M.R. Issue# 864 Comento linea para que mantenga la fecha del comprobante actual, y no la fecha del dia de hoy.
		reverse.setDateDoc(this.getDateDoc()); 
		reverse.set_ValueNoCheck ("C_Period_ID", null);		//	reset
		//reverse.setDateAcct(reverse.getDateDoc());OpenUp M.R. Issue# 864 Comento linea para que mantenga la fecha de contabilizacion del comprobante actual, y no la fecha del dia de hoy.
		...
		// OpenUp 22-03-2011 M.R. Issue#516 Completo asientos contables de la reversion de asientos
		reverse.completeIt();
		reverse.setProcessed(true);
		reverse.setDocStatus(DOCSTATUS_Completed);
	...
	/***
	 * Obtiene y retorna documento para asiento diario simple.
	 * OpenUp Ltda. Issue #29 
	 * @author Gabriel Vila - 17/09/2012
	 * @see
	 * @param ctx
	 * @param trxName
	 * @return
	 */
	public static MDocType getSimpleJournalDocType(Properties ctx, String trxName){
		 
		String whereClause = X_C_DocType.COLUMNNAME_DocBaseType + "='GLJ'" +
							" AND " + X_C_DocType.COLUMNNAME_DocSubTypeSO + "='SJ'";
		MDocType doc = new Query(ctx, I_C_DocType.Table_Name, whereClause, trxName)
		.setClient_ID()
