...
public class MTax extends X_C_Tax
{
	...
	protected boolean beforeSave(boolean newRecord) {
		...
		//OpenUp. Nicolas Sarlabos. 02/09/2013. #1264. No permito mas de una tasa marcada con check de contribuyente final.
		if(isContribuyenteFinal()){
			String whereClause = MTax.COLUMNNAME_ContribuyenteFinal + " = 'Y' AND " + 
			...
			.setOnlyActiveRecords(true)
			.list();
			if (list.size() >= 1) throw new AdempiereException("Ya existe una tasa de impuesto marcada con check 'Contribuyente Final'");
		}
		//Fin OpenUp. #1264
	 ...
	 * OpenUp Ltda. Issue #1202 
	 * @author Gabriel Vila - 09/08/2013
	 * @see
	 * @param ctx
	 * @param value
	 * @param trxName
	 * @return
	 */
	public static MTax forValue(Properties ctx, String value, String trxName){
		
		String whereClause = X_C_Tax.COLUMNNAME_Value + "='" + value + "' ";

		MTax model = new Query(ctx, I_C_Tax.Table_Name, whereClause, trxName)
		.setClient_ID()
		.first();
		
		if (model == null){
			model = new Query(ctx, I_C_Tax.Table_Name, whereClause, trxName)
			.first();
		}
