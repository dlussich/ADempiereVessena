...
import org.openup.model.MPOSectionCategory;
import org.openup.model.MPOSectionProduct;
/**
...
public class MRequisitionLine extends X_M_RequisitionLine
{
	...
	public void setPrice()
	{
		...
		if (m_M_PriceList_ID == 0){
			...
			// OpenUp. Gabriel Vila. 15/02/2014. Issue #92
			// Comento y ahora tomo lista de precio de esta linea y no del cabezal
			
			// m_M_PriceList_ID = getParent().getM_PriceList_ID();
			m_M_PriceList_ID = this.getM_PriceList_ID();
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		// OpenUp. Gabriel Vila. 03/11/2012. Issue #88.
		// Valido que producto este habilitado segun productos por categoria por sector.
		// Recordar que si no se indico producto en categoria de sector, significa que se admiten todos.
	...
	/***
	 * Valida que el producto sea valido para una solicitud de compra en el sector.
	 * Recordar que si no se indico producto en categoria de sector, significa que se admiten todos.
	...
	private void validateProduct() {
		...
		MPOSection section = new MPOSection(getCtx(), this.getParent().getUY_POSection_ID(), null);
		MPOSectionCategory sectcat = section.getSectionCategory(prod.getM_Product_Category_ID()); 
		
		List<MPOSectionProduct> sectprods = sectcat.getSectionProducts();
		
		if (sectprods.size() <= 0) return;
		
		boolean valido = false;
		for (MPOSectionProduct sectprod: sectprods){
			if (sectprod.getM_Product_ID() == this.getM_Product_ID())
				valido = true;
		}
		
		if (!valido){
			throw new AdempiereException("Producto no habilitado para compras de este Sector.");
		}
