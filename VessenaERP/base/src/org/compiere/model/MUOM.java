...
public class MUOM extends X_C_UOM
{
	...
	/**
	 * OpenUp.	
	 * Descripcion : Obtiene ID de UOM segun Symbolo.
	 * @param UOMSymbol
	 * @return
	 * @author  Gabriel Vila 
	 * Fecha : 02/01/2011
	 */
	public static int getIDFromSymbol(String UOMSymbol){
		
		if (UOMSymbol==null) return -1;
		...
		try{
			...
			pstmt.setString(1, UOMSymbol.trim());
	...
	/**
	 * OpenUp.	
	 * Descripcion : Get dentro de una transaction.
	 * @param ctx
	 * @param C_UOM_ID
	 * @return
	 * @author  Gabriel Issue 580
	 * Fecha : 31/03/2011
	 */
	public static MUOM get (Properties ctx, int C_UOM_ID, String trxName)
	{
		if (s_cache.size() == 0)
			loadUOMs (ctx);
		//
		...
		if (uom != null)
			return uom;
		//
		uom = new MUOM (ctx, C_UOM_ID, trxName);
		s_cache.put(C_UOM_ID, uom);
		return uom;
	}	//	get
