...
import org.openup.model.IDynamicHeader;
...
public class GridTab implements DataStatusListener, Evaluatee, Serializable
{
	...
	public HashMap<Integer, GridController> includedTabController = null;
	// Fin OpenUp. Issue #1147.
	
	// OpenUp. Gabriel Vila. 06/09/2013. Issue #1287.
	// Atributo donde guardar el numero de LinkTab padre.
	private GridTab parentLinkTab = null;
	private GridTab childLinkTab = null;
	// Fin OpenUp. Issue #1287

	// OpenUp. Gabriel Vila. 06/09/2013. Issue #1288.
	private MTab tabModel = null;
	...
	// OpenUp. Gabriel Vila. 30/08/2013. Issue #1263.
	...
	public GridTab(GridTabVO vo, GridWindow w, boolean virtual)
	{
		...
		// OpenUp. Gabriel Vila. 30/08/2013. Issue #1263.
		// Asociacion de GridTab con GridTable
		m_mTable.setTabModel(this);
		// Fin OpenUp. Issue #1263.
	...
	public void setQuery(MQuery query)
	{
		if (query == null)
			...
			// OpenUp. Gabriel Vila. 24/10/2013. Issue #1450
			// Si el po implementa la interface para busqueda potenciada
			if (this.m_po == null){
				if (m_mTable != null){
					if (m_mTable.getAD_Table() != null){
						if (!m_mTable.getAD_Table().isView()){
							m_po =  m_mTable.getAD_Table().getPO(0, null);	
						}
					}
				}
			}
			if (this.m_po != null){
				if (this.m_po instanceof IDynamicTask){
					((IDynamicTask)this.m_po).changeMQuery(m_query);
				}
			}
			// Fin OpenUp. Issue #1450.
		...
		if (isDetail())
		{
			...
			else
			{
				...
				{	
					...
					// OpenUp. Gabriel Vila. 06/09/2013. Issue #1287.
					// Si esta tab esta linkeada con otro GridTab Padre
					if (this.parentLinkTab != null){
						value = Env.getContext(m_vo.ctx, m_vo.WindowNo, this.parentLinkTab.getTabNo(), lc, true);
						if (value == null || value.length() == 0)
							value = Env.getContext(m_vo.ctx, m_vo.WindowNo, lc, true); // back compatibility
					}
					// Fin OpenUp. Issue #1287.					
	...
	public void dataRefreshAll (boolean fireEvent)
	{
		...
		m_mTable.dataRefreshAll(fireEvent);//-->Linea original Issue#4887
	...
	private int setCurrentRow (int newCurrentRow, boolean fireEvents)
	{
		...
		// OpenUp. Gabriel Vila. 18/09/2013. Issue #1263.
		// Guardo Record_ID
		int recordID = -1;
		// Fin OpenUp. Gabriel Vila. Issue #1263.
		...
		for (int i = 0; i < size; i++)
		{
			...
			{
				// OpenUp. Gabriel Vila. 18/09/2013. Issue #1263.
				// Guardo value posible para Record_ID
				if (mField.isKey()){
					if (m_mTable.getValueAt(m_currentRow, i) != null){
						recordID = (Integer)m_mTable.getValueAt(m_currentRow, i);	
					}
				}
				// Fin OpenUp. Gabriel Vila. Issue #1263.
...
//					value = mField.getDefault();
			...
			// OpenUp. Gabriel Vila. 19/07/2013. Issue #1147.
			// Aca se setea la visualizacion de los paneles colapsables de las pestañas incluidas segun Logica de Despliegue
			// de dicha pestaña. Hasta hoy las pestañas incluidas se mostraban siempre.
			if (mField.getIncluded_Tab_ID() > 0){
				if (includedTabController != null){
					GridController gc = includedTabController.get(mField.getIncluded_Tab_ID());
					if (gc != null){
						if (gc.getMTab() != null){
							if (gc.getMTab().getIncludedTabs() != null){
								VPanel vpan = gc.getvPanel();
								if (vpan != null){
									Hashtable<Integer, CollapsiblePanel> colaps = vpan.getCollapsedIncludedTab();
									for (GridTab includedTab: gc.getMTab().getIncludedTabs()){
										CollapsiblePanel colap = colaps.get(includedTab.getAD_Tab_ID());
										String dl = includedTab.getDisplayLogic(); 
										if ( dl != null){
											if (!Evaluator.evaluateLogic(gc, dl)){
												colap.setVisible(false);
											}
											else{
												colap.setVisible(true);
											}
										}
									}
								}
		...
		// OpenUp. Gabriel Vila. 30/08/2013. Issue #1263.
		// Asociacion de GridTab con GridTable con PO
		if (recordID > 0){
			MTable table = m_mTable.getAD_Table();
			if (table != null){
				if (!table.isView()){
					m_po =  table.getPO(recordID, null);
					if (m_po != null){
						m_po.setTableModel(m_mTable);
					}
				}
			}
		}
	...
	/**
	 * OpenUp. Gabriel Vila. 06/09/2013. Issue #1287.
	 * @return the parentLinkTab
	 */
	public GridTab getParentLinkTab() {
		return parentLinkTab;
	}

	/**
	 * OpenUp. Gabriel Vila. 06/09/2013. Issue #1287. 
	 * @param parentLinkTab the parentLinkTab to set
	 */
	public void setParentLinkTab(GridTab parentLinkTab) {
		this.parentLinkTab = parentLinkTab;
	}

	/**
	 * OpenUp. Gabriel Vila. 06/09/2013. Issue #1287. 
	 * @return the childLinkTab
	 */
	public GridTab getChildLinkTab() {
		return childLinkTab;
	}

	/**
	 * OpenUp. Gabriel Vila. 06/09/2013. Issue #1287. 
	 * @param childLinkTab the childLinkTab to set
	 */
	public void setChildLinkTab(GridTab childLinkTab) {
		this.childLinkTab = childLinkTab;
	}
	 ...
	 */
	public void setTabModel(MTab tabModel) {
	...
	/**
     * OpenUp. Gabriel Vila. 10/10/2013. Issue #1173 
	 * @param m_gridController the m_gridController to set
	 */
	public void setGridController(GridController m_gridController) {
		this.m_gridController = m_gridController;
	}


	/***
	 * Metodo que obtiene valor de AD_Table_ID si existe como columna de esta grilla.
	 * OpenUp Ltda. Issue #1945 
	 * @author Gabriel Vila - 09/03/2014
	 * @see
	 * @return
	 */
	public int getInlineTable_ID(){
		
		int id = 0;
		
		if (this.m_po.get_Value("AD_Table_ID") != null){
			id = ((Integer)this.m_po.get_Value("AD_Table_ID")).intValue(); 
		}
		return id;
	}

	/***
	 * Metodo que obtiene valor de Record_ID si existe como columna de esta grilla.
	 * OpenUp Ltda. Issue #1945 
	 * @author Gabriel Vila - 09/03/2014
	 * @see
	 * @return
	 */
	public int getInlineRecord_ID(){
		
		int id = 0;
		
		if (this.m_po.get_Value("Record_ID") != null){
			id = ((Integer)this.m_po.get_Value("Record_ID")).intValue(); 
		}
		return id;
	}
	...
	/***
	 * Si el modelo asociado a esta tab requiere manejo de headers de manera dinamica, se procesa aca.
	 * OpenUp Ltda. Issue #2082. 
	 * @author Gabriel Vila - 20/04/2014
	 * @see
	 */
	public void setDynamicFieldsHeader(){
		...
		if (this.m_po != null){
			if (this.m_po instanceof IDynamicHeader){
				((IDynamicHeader)this.m_po).setFielsdHeaderText(this);
			}
		}
