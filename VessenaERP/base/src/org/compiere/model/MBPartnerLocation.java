...
public class MBPartnerLocation extends X_C_BPartner_Location
{
	...
	{
		//OpenUp. Leonardo Boccone. 08/08/2014. Issue #2689
		// lleno la c_location y la asigno a la C_bpartner_location
		if (this.getC_Location_ID() <= 0){
			MLocation c_location = new MLocation(this.getCtx(),0,this.get_TrxName());
			c_location.setC_Country_ID(this.getC_Country_ID());
			c_location.setUY_Departamentos_ID(this.getUY_Departamentos_ID());
			c_location.setUY_Localidades_ID(this.getUY_Localidades_ID());
			c_location.setAddress1(this.getAddress1());
			c_location.setPostal(this.getPostal());
			c_location.saveEx();
			this.setC_Location_ID(c_location.get_ID());
		}
		else{
			MLocation c_location = new MLocation(this.getCtx(), this.getC_Location_ID(), this.get_TrxName());
			if(this.getC_Country_ID()==0){	
				MOrg org = new MOrg(this.getCtx(),this.getAD_Org_ID(),this.get_TrxName());
				MLocation orgLoc = new MLocation(this.getCtx(), org.getInfo().getC_Location_ID(), this.get_TrxName());
				c_location.setC_Country_ID(orgLoc.getC_Country_ID());
			}
			else{
				c_location.setC_Country_ID(this.getC_Country_ID());
			}
	...
	protected boolean afterSave(boolean newRecord, boolean success) {
		...
		//INI OpenUp. SBouissa 01-10-2015 Issue#4857 Nuevo
		if (MSysConfig.getBooleanValue("UY_RETAIL_INTERFACE", false, this.getAD_Client_ID())) {
			if(success && newRecord){
				MRTInterfaceBP mBP = null;
				//Si es nuevo registro y es cliente debo insertar update en la tabla interfazbp para indicar actualizacion 
				if(this.getC_BPartner().isCustomer() && this.isActive()){
						//Busco registro ya sea insert o update (actionId = 0)
						mBP =  MRTInterfaceBP.forClientNotRead(getCtx(), this.get_ID(), 0 ,get_TrxName());
						if(null!=mBP){
							mBP.setC_BPartner_Location_ID(this.get_ID());
							mBP.setProcessingDate(new Timestamp( System.currentTimeMillis()));
						}else{
							mBP = new MRTInterfaceBP(getCtx(), 0, get_TrxName());
							mBP.setC_BPartner_ID(this.getC_BPartner_ID());
							mBP.setC_BPartner_Location_ID(this.get_ID());
							mBP.setattr_1(1);//1 - Si opera como cliente de cuenta corriente con crédito. Esto habilita los bits 2 y 3 y obliga a que el campo CODIGOTIPOCLIENTE tenga valor (y no null).
							mBP.setattr_2(1);//1 - Esta habilitado para pagar con cuenta corriente.
							mBP.setattr_3(1);//1 - Esta habilitado para pagar con el resto de los medios de pago-moneda.
							mBP.setUY_RT_Action_ID(MRTAction.forValue(getCtx(), "update", null).get_ID());
							mBP.setProcessingDate(new Timestamp( System.currentTimeMillis()));
						}
					mBP.saveEx();
				}
				
			}//FIN OpenUp. SBouissa 01-10-2015 Issue#4857 Nuevo
			//INI OpenUp. SBouissa 01-10-2015 Issue#4857 NO es Nuevo
			if (success && !newRecord){
				MRTInterfaceBP mBP = null;
				// Debo insertar registro en la tabla interfazbp para indicar la actualizacion de datos del cliente en sisteco
				// Si se modifica
				if(this.getC_BPartner().isCustomer() && this.isActive()){
					if(is_ValueChanged(X_C_BPartner_Location.COLUMNNAME_Address1)|| 
							is_ValueChanged(X_C_BPartner_Location.COLUMNNAME_department)||
							is_ValueChanged(X_C_BPartner_Location.COLUMNNAME_UY_Departamentos_ID)||
							is_ValueChanged(X_C_BPartner_Location.COLUMNNAME_UY_Localidades_ID)||
							is_ValueChanged(X_C_BPartner_Location.COLUMNNAME_Postal)||
							is_ValueChanged(X_C_BPartner_Location.COLUMNNAME_Phone)){
							//Busco registro ya sea insert o update (actionId = 0)
							mBP =  MRTInterfaceBP.forClientNotRead(getCtx(), this.get_ID(), 0,get_TrxName());
							if(null!=mBP){//si existe seteo valor
								mBP.setC_BPartner_Location_ID(this.get_ID());
								mBP.setProcessingDate(new Timestamp( System.currentTimeMillis()));
							}else{//Si no existe creo un registro update
								mBP = new MRTInterfaceBP(getCtx(), 0, get_TrxName());
								mBP.setC_BPartner_ID(this.getC_BPartner_ID());
								mBP.setC_BPartner_Location_ID(this.get_ID());
								mBP.setattr_1(1);//1 - Si opera como cliente de cuenta corriente con crédito. Esto habilita los bits 2 y 3 y obliga a que el campo CODIGOTIPOCLIENTE tenga valor (y no null).
			...
			}//FIN OpenUp. SBouissa 01-10-2015 Issue#4857 NO es Nuevo
