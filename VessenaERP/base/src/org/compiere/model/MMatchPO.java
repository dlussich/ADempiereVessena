...
public class MMatchPO extends X_M_MatchPO
{
	...
	// OpenUp. Nicolas Sarlabos 12/04/2012 Process Message
	private String m_processMsg = null;

	/**
	 * 	OpenUp. Nicolas Sarlabos 12/04/2012 Get Process Message
		...
		Timestamp dateTrx, BigDecimal qty)
	{
		...
		// OpenUp. Gabriel Vila. 24-02-2011. Issue #140.
		if (retValue!=null) retValue.setPosted(true);
		// Fin OpenUP.
	...
	public MMatchPO (Properties ctx, int M_MatchPO_ID, String trxName)
	{
		...
		//	setQty (Env.ZERO);
			setPosted (true); // OpenUp M.R. 15-02-2011 issue #138 cambio el estado de la aplicacion po defecto a true
			setProcessed (false);
			setProcessing (false);
		}
	}	//	MMatchPO
		...
		if (getM_InOutLine_ID() != 0)
		{
			...
			for(int asn = 0; asn < acctschemas.length; asn++)
			{
				...
				if (oLine.getC_Currency_ID() != as.getC_Currency_ID())
				{
					...
					//Se obtiene fecha de contabilizacion sin hora, de lo contrario no se puede obtener tasa de cambio.
					...
					if (rate == null)
					{	
						//OpenUp Nicolas Sarlabos 12/04/2012, se controla que exista tasa de cambio para la fecha de la orden
						MCurrency cur = new MCurrency(getCtx(), order.getC_Currency_ID(), null);
						this.m_processMsg = "No se encuentra tipo de cambio para fecha : " + dateAcct.toString() +
											", Moneda : " + cur.getDescription() + 
											", utilizada en la Orden de Compra Numero: " + order.getDocumentNo();
						// Fin OpenUp Nicolas Sarlabos 12/04/2012
