...
public class MLookupFactory
{
		...
		int WindowNo, int AD_Reference_Value_ID)
	{
		...
		// OpenUp. Gabriel Vila. 24/02/2011. Issue #59.
		// Comento if para cache ya que no contempla cambios en contexto.
		/*if (retValue != null)
		{
	...
	{
		// OpenUp. Gabriel Vila. 10/07/2007.
		// Para busquedas donde se discrimina cliente o proveedor, se agrega la terminacion "_P" 
		// al campo C_BPartner_ID. Pero esto sirve para cualquier entidad. Con esta trampa
		// asocio la misma busqueda que la columna original.
		if (ColumnName.endsWith("_ID_P")){
			String aux = ColumnName.replace("_ID_P", "_ID");
			ColumnName = aux;
		}
		else{
