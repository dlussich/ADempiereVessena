...
public class MCost extends X_M_Cost
{
		...
		boolean zeroCostsOK, String trxName)
	{
		...
		// Me cubro que materialCost no sea null
		if (materialCost == null) materialCost = Env.ZERO;
		// Fin OpenUp
	...
	public static void create (MClient client)
	{
		...
		try
		{
			...
			// OpenUp. Gabriel Vila. 14/06/2012. Issue#986
			// Recalcula costos.
			if (MSysConfig.getBooleanValue("UY_COST_CALCULATE_AUTO", false)){
				MProductCost.calculateCosts(0, true);	
			}
			// Fin OpenUp.
	 ...
	 * 
	 * OpenUp. #944	
	 * Descripcion : Metodo que busca el costo de un producto en tabla de costos ajustados y lo devuelve 
	 * @author  Nicolas Sarlabos
	 * Fecha : 14/02/2012
	 */

	public static BigDecimal getCostFromTable(int prodId, Timestamp date) {
		...
		if (prodId > 0) {
			...
			String sql = "SELECT cal.amount FROM uy_costadjustmentline cal"
					+ " INNER JOIN uy_costadjustment ca ON cal.uy_costadjustment_id=ca.uy_costadjustment_id" + " WHERE ca.datetrx =" + "'" + date + "'"
					+ " AND ca.docstatus='CO' AND cal.m_product_id=" + prodId + " ORDER BY cal.updated desc";

			cost = DB.getSQLValueBDEx(null, sql);
