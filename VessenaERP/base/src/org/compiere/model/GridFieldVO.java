		...
		int AD_Window_ID, int AD_Tab_ID, boolean readOnly, ResultSet rs)
	{
		...
		try
		{
			...
			for (int i = 1; i <= rsmd.getColumnCount(); i++)
			{
				...
				// OpenUp. Gabriel Vila. 26/06/2013. Issue #1074.
				// Poder utilizar variables de contexto en texto de columnas virtuales !!!! 
				// Por ahora uso solo para el usuario.
				// Comento y sustituyo.
				/*
				...
				*/	
					...
					vo.ColumnSQL = rs.getString(i);
					if ((vo.ColumnSQL != null) && (!vo.ColumnSQL.equalsIgnoreCase(""))){
						if (vo.ColumnSQL.contains("@#AD_User_ID@")){
							vo.ColumnSQL = vo.ColumnSQL.replace("@#AD_User_ID@", Env.getContext(Env.getCtx(), "#AD_User_ID"));
						}
					}
				}				
				// Fin OpenUp. Issue #1074.
