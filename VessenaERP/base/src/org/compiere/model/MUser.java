...
public class MUser extends X_AD_User
{
	...
	//--26/03/2014 
		/**
		 *  OpenUP Sylvie Bouissa se crea para verificación de usuario desde webserver
		 * 	Get boolean 
		 *	@param name name
		 *	@param password password
		 *	@return true or false
		 */
		public static boolean get ( String name, String password)
		{
			if (name == null || name.length() == 0 || password == null || password.length() == 0)
			{
				s_log.warning ("Invalid Name/Password = " + name + "/" + password);
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		// OpenUp. Gabriel Vila. 15/07/2013. Issue #1138.
		// Valido que no tenga otro usuario con el mismo nombre
		// Antes valido que se haya ingresado un nombre
		if ((this.getName() == null) || (this.getName().equalsIgnoreCase(""))){
			throw new AdempiereException("Debe indicar Nombre de Usuario.");
		}
		
		if (newRecord){
			MUser otro = MUser.forName(getCtx(), this.getName().trim(), null);
			if ((otro != null) && (otro.get_ID() > 0)){
				throw new AdempiereException("Ya existe un Usuario con este Nombre.");
			}
		}
		// Fin OpenUp. Issue #1138.
	...
	/***
	 * OpenUp. Agrego el metodo afterSave a este modelo.
	 * 12/03/2014. Issue #1260.
	 */
	@Override
	protected boolean afterSave(boolean newRecord, boolean success) {
		...
		if (is_ValueChanged("DelegateTasks")){
			// Si este usuario delega tareas a otro
			if (this.get_ValueAsBoolean("DelegateTasks")){
				if (this.get_ValueAsInt("DelegateUser_ID") > 0){
					// Delego sus tareas activas al nuevo usuario
					MRInbox.delegateUserTasks(getCtx(), this.get_ID(), this.get_ValueAsInt("DelegateUser_ID"), true, null);
				}
			}
			else{
				// Este usuario dejo de delegar tareas, tengo que poner las tareas aun activas de nuevo a él
				MRInbox.delegateUserTasks(getCtx(), this.get_ID(), this.get_ValueAsInt("DelegateUser_ID"), false, null);
			}
		}
	...
	/***
	 * Obtiene modelo segun nombre recibido.
	 * OpenUp Ltda. Issue #1138 
	 ...
	 * @return
	 */
	public static MUser forName(Properties ctx, String name, String trxName){
		
		String whereClause = " lower(name) ='" + name + "'";
		
		MUser model = new Query(ctx, I_AD_User.Table_Name, whereClause, trxName)
		.setClient_ID()
		.first();
	...
	/***
	 * Obtiene modelo segun nombre recibido y sin discriminar empresa.
	 * OpenUp Ltda. Issue #1138 
	 * @author Gabriel Vila - 15/07/2013
	 * @see
	 * @param ctx
	 ...
	 * @return
	 */
	public static MUser forNameAndSystem(Properties ctx, String name, String trxName){
