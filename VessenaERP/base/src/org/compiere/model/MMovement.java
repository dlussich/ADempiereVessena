...
public class MMovement extends X_M_Movement implements DocAction
{
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		//OpenUp. Nicolas Sarlabos. 30/08/2013. #1254. DEFENSIVO: me aseguro que esten bien seteados los campos de todas las lineas luego de guardar para 
		//el documento de transferencia de fabrica a logistica, por si se cambio el tipo de documento.
		...
		if(doc.getValue()!=null){
			if(doc.getValue().equalsIgnoreCase("transfablog")){
				...
				if(status.get_ID()>0){
					...
					String sql = "update m_movementline set uy_stockstatus_id = " + status.get_ID() + ",uy_stockstatus_to_id = " + status.get_ID() +
							",m_locator_id = 1000002,m_locatorto_id=1000012 where m_movement_id = " + this.get_ID();
