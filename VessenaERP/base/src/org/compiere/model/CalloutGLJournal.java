...
public class CalloutGLJournal extends CalloutEngine
{
	...
	public String rate (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		if (DateAcct == null){
			...
			// OpenUp. Gabriel Vila. 15/03/2013. Issue #548
			// El objeto mTab pertenece a la Linea del asiento y cuando hago nueva linea nunca
			// tiene la DateAcct seteada. Pero en vez de tomar la del dia, deberia tomar la DateAcct
			// indicada en el cabezal (GL_Journal)
			int journalID = Env.getContextAsInt (ctx, WindowNo, 0, "GL_Journal_ID");
			if (journalID > 0){
				DateAcct = new MJournal(ctx, journalID, null).getDateAcct();
			}
			// Fin Issue #548.
