...
public class MBank extends X_C_Bank
{
	 ...
	 * @return
	 */
	public static MBank getFromAccount(Properties ctx, int account_ID) {

		MBank bank = null;

		if(account_ID > 0){

			MBankAccount accountbank = new MBankAccount(ctx,account_ID,null);

			if(accountbank.get_ID()>0) {

				bank = new MBank(ctx,accountbank.getC_Bank_ID(),null);
			}
		}
