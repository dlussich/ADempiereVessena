...
//		 Bug 1353695 Currency Rate and COnbversion Type should get copied from journal to lines
		for (int i = 0; i < journals.length; i++) 
		{
			...
			if (journal.getCurrencyRate() != null && journal.getCurrencyRate().compareTo(Env.ZERO) != 0)
			{
				for (int j = 0; j < lines.length; j++) 
				{
					...
					//line.save(); // OpenUp. Gabriel Vila. 07/06/2011. Issue #714. Comento porque da error y no es relevante.
				}
			}
	...
	public boolean voidIt()
	{
		...
		//OpenUp. Nicolas Sarlabos. 10/06/2013. #789
		MJournal[] journals = this.getJournals(true); //obtengo asientos a anular
		
		for (int i = 0; i < journals.length; i ++){
			
			MJournal j = new MJournal (getCtx(),journals[i].get_ID(),get_TrxName());
			
			if (!j.processIt(DocumentEngine.ACTION_Void)) throw new AdempiereException(j.getProcessMsg());			
	...
	public boolean reverseAccrualIt()
	{
		...
		if (description == null)
			...
			description += " ** " + getDocumentNo() + " **";
		reverse.processIt(MJournal.DOCACTION_Reverse_Correct); //OpenUp M.R. 29-08-2011 Issue# 864 actualizo accion del documento 
		reverse.setProcessed(true);//OpenUp M.R. 29-08-2011 Issue# 864 agrego linea para dejar el documento procesado
		reverse.setDescription(description);
		...
		this.setDocAction(DOCACTION_None);//OpenUp M.R. 18-03-2011 Issue#517 seteo boton a sin accion 
