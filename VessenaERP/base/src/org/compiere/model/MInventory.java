...
public class MInventory extends X_M_Inventory implements DocAction
{
	...
	public String completeIt()
	{
		...
		for (MInventoryLine line : lines)
		{
			...
			//OpenUp Nicolas Sarlabos 27/04/2012 #1010, se modifica para obtener la cantidad por tipo de estado stock
			// OpenUp. Gabriel Vila. 17/02/2012. Issue #719.
			// Uso Modelo de Stock de OpenUp.
			line.setQtyBook(MStockTransaction.getQtyByStatus(0, line.getM_Locator_ID(), line.getM_Product_ID(), 
							line.getM_AttributeSetInstance_ID(), this.getMovementDate(), line.getUY_StockStatus_ID(), get_TrxName()));
			// Fin OpenUp
			//Fin OpenUp Nicolas Sarlabos 27/04/2012 #1010
	...
	public boolean voidIt()
	{
		...
		// OpenUp. Gabriel Vila. 05/04/2011. Issue #832.
		/*else
		{
		...
		setProcessed(true);
		setDocStatus(DocAction.STATUS_Voided); // Issue #832. Agrego esta linea.
	...
	private String createCostDetail(MInventoryLine line, int M_AttributeSetInstance_ID, BigDecimal qty)
	{
		...
		for(int asn = 0; asn < acctschemas.length; asn++)
		{
			...
			if (costs == null)
			{
				...
				// Cambio retorno porque me da problemas por ahora el tema de costo de inventario.
	...
	/***
	 * Obtiene y retorna ultimo inventario en estado completo.
	 * OpenUp Ltda. Issue #971 
	 * @author Gabriel Vila - 10/07/2012
	 * @see
	 * @return
	 */
	public static MInventory getLastInventory() {

		final String whereClause = " AD_Client_ID = ? AND AD_Org_ID = ? AND DocStatus=? ";
		final String orderBy = " MovementDate DESC ";
		MInventory value = new Query(Env.getCtx(), I_M_Inventory.Table_Name, whereClause, null)
						   .setParameters(Env.getAD_Client_ID(Env.getCtx()), Env.getAD_Org_ID(Env.getCtx()), DocumentEngine.STATUS_Completed)
						   .setOrderBy(orderBy)
						   .first();
		return value;
	}
