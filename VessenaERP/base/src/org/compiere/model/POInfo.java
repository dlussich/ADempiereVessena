	...
	public String getColumnSQL (int index)
	{
		...
		*/
		if (m_columns[index].ColumnSQL != null && m_columns[index].ColumnSQL.length() > 0){
			...
			if (m_columns[index].ColumnSQL.contains("@#AD_User_ID@")){
				m_columns[index].ColumnSQL = m_columns[index].ColumnSQL.replace("@#AD_User_ID@", Env.getContext(Env.getCtx(), "#AD_User_ID"));
			}
