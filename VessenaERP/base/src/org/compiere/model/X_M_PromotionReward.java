...
// @SQL=SELECT COALESCE(MAX(SeqNo),0)+10 AS DefaultValue FROM M_PromotionReward WHERE M_Promotion_ID=@M_Promotion_ID@
	...
	/** Set uy_auximporte.
		...
		uy_auximporte
	  */
	public void setuy_auximporte (BigDecimal uy_auximporte)
	{
		set_Value (COLUMNNAME_uy_auximporte, uy_auximporte);
	}

	/** Get uy_auximporte.
		@return uy_auximporte
	  */
	public BigDecimal getuy_auximporte () 
	{
		BigDecimal bd = (BigDecimal)get_Value(COLUMNNAME_uy_auximporte);
		if (bd == null)
			 return Env.ZERO;
		return bd;
	}
