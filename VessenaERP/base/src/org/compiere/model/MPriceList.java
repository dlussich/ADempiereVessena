...
public class MPriceList extends X_M_PriceList
{
	 ...
	 */
	public static MPriceList getDefault (Properties ctx, boolean IsSOPriceList, int cCurrencyID)
	{
		...
		while (it.hasNext())
		{
			...
			if (retValue.isDefault()
					...
					&& retValue.getC_Currency_ID() == cCurrencyID)
			{
				return retValue;
			}
		}
		...
		if(cCurrencyID>0){
			whereClause += " AND C_Currency_ID = "+cCurrencyID;
		}else{
			whereClause += " AND C_Currency_ID = "+ OpenUpUtils.getSchemaCurrencyID(ctx, AD_Client_ID, null);
		}
	...
	public static MPriceList getDefault(Properties ctx, boolean IsSOPriceList, String ISOCurrency)
	{
		...
		if (currency==null) return(getDefault(ctx, IsSOPriceList,0));
	 ...
	 * 
	 * OpenUp. issue #867	
	 * Descripcion : Metodo similar al anterior, pero que ademas recibe el Id de la ultima version 
	 ...
	 * Fecha : 16/09/2011
	 */
	public MPriceListVersion getPriceListVersion (Timestamp valid,int id)
	{
		String whereClause = "";
		if (valid == null)
			valid = new Timestamp (System.currentTimeMillis());
		if(id <= 0){
			whereClause = "M_PriceList_ID=? AND TRUNC(ValidFrom)<=? AND IsActive=?";
		}else{
			whereClause = "M_PriceList_ID=? AND TRUNC(ValidFrom)<=? AND IsActive=? AND M_PriceList_Version_ID=" + id;
	...
	/***
	 * Obtiene y retorna la version VIGENTE de esta lista de precios, para una
	 * fecha dada. si no encuentra version vigente para esa fecha devuelve nulo
	 * OpenUp Ltda. Issue #5427
	 * 
	 * @author INes Fernandez - 16/07/2015
	 * @see
	 * @param ctx
	 * @param date: fecha para la que se consulta
	 * @param trxName
	 * @return
	 */
	public MPriceListVersion getVersionVigente(Timestamp date) {

		MPriceListVersion version = null;
		if (date == null) date = TimeUtil.trunc(new Timestamp (System.currentTimeMillis()), TimeUtil.TRUNC_DAY);

		String sql = "SELECT max (M_PriceList_Version_ID)" +
                     " FROM M_PriceList_Version" + 
                     " WHERE M_PriceList_Version.isActive = 'Y'" +
                     " AND M_PriceList_Version.M_PriceList_ID = " + this.get_ID() + 
                     " AND validFrom <= '" + date + "'";

		int versionID = DB.getSQLValueEx(get_TrxName(), sql);

		if (versionID > 0)
			version = new MPriceListVersion(getCtx(), versionID, get_TrxName());

		return version;
	}

	/**
	 * Rertorna lista de preciod e venta para la organización y moneda correspondiente
	 * @author OpenUp SBT Issue #5386  28/1/2016 17:37:34
	 * @param ctx
	 * @param m_OrgID
	 * @param trxName
	 * @return
	 */
	public static MPriceList getPricListForOrg(Properties ctx, int m_OrgID, int c_CurrencyID,
			String trxName) {
		
		MPriceList pl = null;
		
		final String whereClause = " AD_Org_ID IN ("+m_OrgID+") AND C_Currency_ID = ("+c_CurrencyID+")"
				+ " AND IsActive = 'Y' AND IsTaxIncluded = 'Y'"
				+ " AND IsSoPriceList = 'Y'"; 
		pl = new Query(ctx, Table_Name, whereClause, trxName).setOnlyActiveRecords(false).first();
