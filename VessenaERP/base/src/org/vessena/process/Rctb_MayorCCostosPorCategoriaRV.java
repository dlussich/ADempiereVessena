...
public class Rctb_MayorCCostosPorCategoriaRV extends SvrProcess {
	...
	public Rctb_MayorCCostosPorCategoriaRV() {
	...
	private String getSQLCategoria(String valueCateg) {
		...
		sql = "SELECT " + this.adClientID + "," + this.adOrgID + "," + this.adUserID + ",'" + this.idReporte +  "',current_date, a.dateacct, cta.name, " +
				  ...
				  " INNER JOIN c_activity act ON act.c_activity_ID = a.c_activity_ID_" + valueCateg +					  
	...
	private void calculoSaldos(){
		...
		try{
			...
			int uyCategCCostoID = 0, cCostoID = 0;
			String valorcuenta = "";
			
			// Obtengo registros de la temporal ordenados por centrosDeCosto - cuenta - fecha - idAsiento - idtabla
			sql = " SELECT a.* " +
				  " FROM " + TABLA_MOLDE + " a " +
				  " inner join c_activity act on  a.c_activity_id = act.c_activity_id " +	
				  " WHERE a.idreporte=?" +
				  " ORDER BY a.uy_categoria_ccostos_id, act.name, a.valorcuenta, a.datetrx, " +
				  " a.idasiento, a.idtabla, a.fact_acct_id ASC";
			...
			while (rs.next()){
				...
				// Obtengo los datos de estos registros				
				int uyCategCCostoIDAux = rs.getInt("uy_categoria_ccostos_id");
				int cCostoIDAux = rs.getInt("c_activity_id");
				String valorCuentaAux = rs.getString("valorcuenta");
				
				if(uyCategCCostoIDAux != uyCategCCostoID){
					
					if(cCostoIDAux != cCostoID){						
	...
	private void updateDatosRegistro(int idCCosto, int c_elementValue_id, Timestamp fechaDoc, int idAsiento, int idTabla,
									 BigDecimal saldoPeriodo, BigDecimal saldoAcumulado, InfoDocumentoBean infoDoc, String nomSocioNeg, int factAcctID){
