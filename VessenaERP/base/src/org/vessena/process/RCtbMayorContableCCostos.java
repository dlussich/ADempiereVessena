 ...
 * Fecha : 02/10/2010
 */
public class RCtbMayorContableCCostos extends SvrProcess {
...
//	private String nombreUsuario = "";
	...
	private static final String TABLA_MOLDE = "UY_MOLDE_RCtbMayorCCostos";
	
	/**
	 * Constructor
	 */
	public RCtbMayorContableCCostos() {
	...
	private void loadMovimientos(){
		...
		String insert = "", sql = "";
		String whereCuentas = "", camposImportes = "";
		String whereCentros = "";
		try
		{
			insert = "INSERT INTO " + TABLA_MOLDE + " (idreporte, idusuario, idcuenta, valorcuenta, nombrecuenta, fecreporte, fecdoc, " + 
					...
					" imporigen, saldoinicial, debe, haber, saldoperiodo, saldoacumulado, idsocionegocio, nombresocionegocio, idtabla, nomtabla, c_activity_id) ";
			
			// Condicion de cuentas
			
			if (this.idCuentaDesde>0) 
				whereCuentas = " AND a.account_id>=" + this.idCuentaDesde;
			if (this.idCuentaHasta>0) 
				whereCuentas += " AND a.account_id<=" + this.idCuentaHasta;
			...
			sql = "SELECT '" + this.idReporte + "'," + this.idUsuario + ", a.account_id, cta.value, cta.name, current_date, a.dateacct, " +
				  ...
				  camposImportes +
				  " 0, 0, a.c_bpartner_id, coalesce(bp.name,'') as bpname, a.ad_table_id, tab.tablename, a.c_activity_id " +
				  " FROM fact_acct a " + 
	...
	private void calculoSaldos(){
		...
		try{
			...
			sql = "SELECT * " +
				  " FROM " + TABLA_MOLDE +
				  " WHERE idreporte=?" +
				  " ORDER BY c_activity_id, valorcuenta, fecdoc,  idasiento, idtabla ASC";
			...
			while (rs.next()){
				...
				if (!valorCuentaAux.equalsIgnoreCase(valorcuenta)){
					...
					if (this.conSaldoInicial){
						...
						saldoInicial = this.getSaldoInicial(rs.getInt("c_activity_id"), rs.getInt("idcuenta"));

						// Actualizo saldo inicial para este nueva cuenta
						this.updateSaldoInicial(rs.getInt("c_activity_id"), saldoInicial, rs.getInt("idcuenta"));
					}
					else{
				...
				this.updateDatosRegistro(rs.getInt("c_activity_id"), rs.getInt("idcuenta"), rs.getTimestamp("fecdoc"), rs.getInt("idasiento"), rs.getInt("idtabla"), 
	 ...
	 * Fecha : 03/10/2010
	 */
	private void updateSaldoInicial(int idCCosto, BigDecimal valor, int idCuenta){
	 ...
	 * Fecha : 03/10/2010
	 */
	private void updateDatosRegistro(int idCCosto, int idCuenta, Timestamp fechaDoc, int idAsiento, int idTabla,
	 ...
	 * Fecha : 28/09/2010
	 */
	private BigDecimal getSaldoInicial(int idCCosto, int idCuenta){
