...
public class MHRAttribute extends X_HR_Attribute
{
	...
	protected boolean beforeSave(boolean newRecord) {
		...
		if(this.getValidTo()!=null){
			if(this.getValidFrom().compareTo(this.getValidTo()) >= 0 ){
			
				throw new AdempiereException("La fecha de validez inicial debe ser menor a la fecha final");
			}
		}
		//se validan los valores
		if(this.getMinValue()>0 && this.getMaxValue()>0){
			if(this.getMinValue() > this.getMaxValue() || this.getMinValue() == this.getMaxValue()){
			
				throw new AdempiereException("El valor mínimo debe ser menor al valor máximo");
