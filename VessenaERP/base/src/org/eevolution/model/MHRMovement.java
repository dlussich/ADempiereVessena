...
public class MHRMovement extends X_HR_Movement
{
	...
	public MHRMovement (X_I_HR_Movement impHRm)
	{
		...
		//Fin Nicolas Sarlabos 19/04/2012
	...
	public void setColumnValue(Object value)
	{
		try
		{
			...
			//Fin OpenUp Nicolas Sarlabos 19/04/2012
			else
			{
	...
	protected boolean beforeSave(boolean newRecord)
	{
		...
		// OpenUp. Gabriel Vila. 06/04/2011. Issue #605
		// Eevolution hizo cambios en la clase org.compiere.model.Query y los mismo no fueron publicados.
		// Por lo tanto modifico codigo para poder traer la cuenta bancaria.
		...
		MBPBankAccount[] banks = MBPBankAccount.getOfBPartner(getCtx(), this.getC_BPartner_ID());
		int C_BP_BankAccount_ID = 0;
		if (banks.length > 0) C_BP_BankAccount_ID = banks[0].getC_BP_BankAccount_ID();
		// Fin OpenUp.
	...
	public static List<MHRMovement> getLinesForProcess (MHRProcess process)
	{
		...
		// OpenUp. Gabriel Vila. 06/04/2011. Issue #605
		// Modifico llamada a Query ya que eevolution modifico la clase y no dejo publicacion
		...
		return new Query (process.getCtx(), I_HR_Movement.Table_Name, whereClause , process.get_TrxName())
		.setClient_ID()
		.setParameters(new Object[] { process.getHR_Process_ID() })
		.list();
