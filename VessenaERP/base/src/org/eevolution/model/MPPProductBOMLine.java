...
public class MPPProductBOMLine extends X_PP_Product_BOMLine
{
	...
	{
		// OpenUp. Nicolas Garcia. 23/08/2011. Issue #837.
		// La linea toma el isActive de el cabezal
		MPPProductBOM ppProduct = (MPPProductBOM) this.getPP_Product_BOM();
		this.setIsActive(ppProduct.isActive());
		// Fin Issue #837
		
		//OpenUp Nicolas Sarlabos 12/06/2012 #1030,si es nuevo registro controlo que el producto hijo no exista en la lista
		if(newRecord){
			String sql = "SELECT coalesce(COUNT(m_product_id),0)" +
					" FROM pp_product_bomline l" +
					" WHERE pp_product_bom_id=" + this.getPP_Product_BOM_ID() +
					" AND m_product_id=" + this.getM_Product_ID();
			int cant = DB.getSQLValueEx(get_TrxName(), sql);
			if(cant > 0) throw new AdempiereException ("Ya existe una linea con ese producto.En caso de necesidad,modificar cantidad en la LDM");
		
		//OpenUp. Nicolas Sarlabos. 25/10/2012
		//se valida que el producto hijo corresponda a la categoria del padre segun la parametrizacion realizada
		/*MProduct prod = new MProduct(getCtx(),ppProduct.getM_Product_ID(),get_TrxName());
		int categoryID = prod.getM_Product_Category_ID();
		MProductCategory category = new MProductCategory(getCtx(), categoryID, get_TrxName());
		MProduct prodHijo = new MProduct(getCtx(),this.getM_Product_ID(),get_TrxName());
		
		sql = "SELECT l.m_product_id" +
		      " FROM UY_ParametrosStdProdLine l" +
		      " INNER JOIN UY_ParametrosStdProdHdr h ON l.UY_ParametrosStdProdHdr_ID=h.UY_ParametrosStdProdHdr_ID " +
		      " WHERE h.m_product_category_id = ? ";
		...
		try{
			...
			if(rs.next()){
				...
				if(this.getM_Product_ID()==rs.getInt(1)) existe=true;
				
			}
			
			if(existe && prodHijo.getProductType().equalsIgnoreCase("S")) throw new AdempiereException("Error: el producto " + prodHijo.getValue() + " no corresponde a la categoría " + category.getName() + " del producto padre");
		...
		//fin OpenUp Nicolas Sarlabos 12/06/2012 #1030
	...
	protected boolean afterSave(boolean newRecord, boolean success)
	{
		...
		// Comento lineas para carga masiva
		...
		//OpenUp Nicolas Sarlabos 09/07/2012 #1042 
		sql = "select coalesce(sum(l.qtybom),0)" +
			  ...
			  " where hdr.pp_product_bom_id=" + this.getPP_Product_BOM_ID() + " and l.m_product_id in (select pl.m_product_id" + 
		...
		sumQty=DB.getSQLValueBDEx(get_TrxName(), sql);
		
		if (sumQty.compareTo(Env.ZERO) > 0){
			
			finalQty = (Env.ONE.divide(sumQty,1, RoundingMode.HALF_UP));
						
			int std_id = 0;
			MPPProductBOM productBOM = (MPPProductBOM) this.getPP_Product_BOM(); //obtengo cabezal
			//obtengo ID del Estandar de Producciones correspondiente
			sql = "SELECT uy_estandar_producciones_id FROM uy_estandar_producciones WHERE m_product_id= " + productBOM.getM_Product_ID();
			std_id = DB.getSQLValue(get_TrxName(), sql);
			//instancio Estandar de Producciones si obtuve ID del mismo	
			if(std_id>0){
				MEstandarProducciones std = new MEstandarProducciones(getCtx(), std_id, get_TrxName()); 

				std.setUY_UnidHoraOp(finalQty);
				std.saveEx();
			}
				
		}
		//Fin OpenUp Nicolas Sarlabos 09/07/2012 #1042
		return true;
	}
