...
public class MPPProductBOM extends X_PP_Product_BOM
{
	...
	{
		// OpenUp Nicolas Garcia 22/06/2011 #734
		// Se cambio la clausula de busqueda desde value a el is active.
		int AD_Client_ID = Env.getAD_Client_ID(product.getCtx());
		String sql = "SELECT PP_Product_BOM_ID FROM PP_Product_BOM"
						+" WHERE isActive='Y' " +
						//"AND Value=? " +
						"AND M_Product_ID=? AND AD_Client_ID=?";
		return DB.getSQLValueEx(null, sql, product.get_ID(), AD_Client_ID);
		// Fin OpenUp Nicolas Garcia 22/06/2011 #734
	}
	...
	protected boolean beforeSave(boolean newRecord) {
		...
		// OpenUp Nicolas Garcia Issue #734 21/06/2011 Actualizado #837
		// 23/08/2011
		//Si lo que se va a guardar esta activo,
		// OpenUp INes Fernandez Issue #4701. 17/08/2015. Prod de Transf admiten mas de una LDM
		if(this.isActive() && !MSysConfig.getBooleanValue("UY_IS_PROD_TRANSF", false, this.getAD_Client_ID()) ){
		//End Issue #4701.
			//MWindow window=new MWindow(Env.getCtx(),Env.,"");
			if (ADialog.ask(0, null, "¿Desea que esta lista sea la Activa?")){

				// Actualizo cabezales.
				String action = "UPDATE " + X_PP_Product_BOM.Table_Name + " SET isActive='N' WHERE m_product_id = " + this.getM_Product_ID()
						+ " and PP_Product_BOM_ID <>" + this.get_ID() + ";";
				
				// Actualizo las lineas de los otros cabezales
				action += " UPDATE " + X_PP_Product_BOMLine.Table_Name
						+ " SET isActive='N' WHERE PP_Product_BOM_ID in ( SELECT PP_Product_BOM_ID from PP_Product_BOM WHERE m_product_id = "
						+ this.getM_Product_ID() + " and PP_Product_BOM_ID <> " + this.get_ID() + ");";

				// Se actualizan las lineas de este cabezal
				action += " UPDATE " + X_PP_Product_BOMLine.Table_Name + "  SET isActive='Y' WHERE PP_Product_BOM_ID =" + this.get_ID() + ";";

				DB.executeUpdate(action,get_TrxName());
				
			}else{//Si no es activa
				this.setIsActive(false);
			}
		}
		
		// Fin Nicolas Garcia Issue #734 21/06/2011		
		return true;
	}
