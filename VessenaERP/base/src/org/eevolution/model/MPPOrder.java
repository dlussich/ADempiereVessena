...
public class MPPOrder extends X_PP_Order implements DocAction
{
	...
	protected boolean beforeSave(boolean newRecord)
	{
		...
		//OpenUp Nicolas Garcia 25/07/2011 Issue #762
		updatePlanClose();		
		//Fin Issue #762
	...
	public boolean processIt(String processAction)
	{
		...
		// OpenUp. Gabriel Vila. 23/03/2011. Issue #534
		// Guardo DocAction actual para tareas posteriores al procesar
		String docAction = getDocAction();
		// Fin OpenUp
		
		m_processMsg = null;
		DocumentEngine engine = new DocumentEngine(this, getDocStatus());
		boolean processOk = engine.processIt(processAction, getDocAction());
		
		// OpenUp. Gabriel Vila. 01/10/2010.
		// Emision automatica del reporte de orden de proceso al completarse la orden
		if (processOk && docAction.equalsIgnoreCase(DocumentEngine.ACTION_Prepare)){
			/*try{

				int idDocType = this.getC_DocTypeTarget_ID();
				int AD_Process_ID = 0;
				
				// Si estoy en una orden de envasado el proceso es para el reporte de envasado
				if (idDocType == 1000043) {
					AD_Process_ID = 1000033;  	
				}
				// Si es orden de orden de fabricacion u orden de reproceso, el proceso es para el reporte de fabricacion 
				else if ( (idDocType == 1000033) || (idDocType == 1000042) ) {
					AD_Process_ID = 1000051;
				}

				MPInstance instance = new MPInstance(Env.getCtx(), AD_Process_ID, 0);
				if (!instance.save())
				{
					log.log(Level.WARNING,"No fue posible guardar informaci�n de instancia para Proceso de Reporte de Orden de Proceso.");
					return processOk;
				}
				ProcessInfo pi = new ProcessInfo ("MPPOrder", AD_Process_ID);
			...
			// OpenUp. Gabriel Vila. 01/03/2011. Issue #394.
			// Obtengo camino del archivo pdf asociado al flujo de produccion
			try{
				MWorkflow wflow = new MWorkflow(getCtx(), this.getAD_Workflow_ID(), null);
	...
	public String prepareIt()
	{
		...
		// Comento codigo que verifica que el alamcen de hdr sea igual al de la
		// lineas

		//	Lines
		MPPOrderBOMLine[] lines = getLines(true);

		// OpenUp. Nicolas Garcia. 09/09/2011. Issue #874.
		// Se permite Disponible negativo?
		if (!MSysConfig.getBooleanValue("UY_STOCK_AVAILABLE_NEGATIVE", true, this.getAD_Client_ID())) {

			// Recorro Lineas
			for (int i = 0; i < lines.length; i++) {

				// Busco Disponible
				BigDecimal disponible = MStockTransaction.getQtyAvailable(lines[i].getM_Warehouse_ID(), lines[i].getM_Locator_ID(), lines[i].getM_Product_ID(),
						lines[i].getM_AttributeSetInstance_ID(), 0, null, this.get_TableName());

				// Si la cantidad a reservar supera a la cantidad disponible
				if (lines[i].getQtyReserved().compareTo(disponible) > 0) {

					MProduct pro = (MProduct) lines[i].getM_Product();

					if (m_processMsg == null) m_processMsg = " Stock disponible insuficiente=> Producto: " + pro.getValue() + ", Disponible: "
							+ disponible.intValue() + ", Requerido: "
							+ lines[i].getQtyReserved().intValue() + "\n";
					else
						m_processMsg += " Stock disponible insuficiente=> Producto: " + pro.getValue() + ", Disponible: " + disponible.intValue()
								+ ", Requerido: "
								+ lines[i].getQtyReserved().intValue() + "\n";
				}
	...
	private void orderStock()
	{
		...
		//OpenUp Nicolas Sarlabos 10/07/2012 #1043, impido cantidad reservada negativa
		BigDecimal reserved = (getQtyReserved().add(difference));
		if(reserved.compareTo(Env.ZERO)<0) reserved = Env.ZERO;
		setQtyReserved(reserved);
	...
	public String completeIt()
	{
		...
		// OpenUp. Gabriel Vila. 08/09/2010.
		// Valido que se hayan cargados los usuarios en las lineas de la orden.
		//m_processMsg = this.validateOrderUserBOMLine(this.getPP_Order_ID());
		//if (m_processMsg != null)
		//	return DocAction.STATUS_Invalid;
		// Fin OpenUp.
		...
		// OpenUp. Gabriel Vila. 16/05/2011. 
		// Pongo posted en true ya que no se postea este documento
		setProcessed(true);
		setPosted(true);
		// Fin OpenUp.
...
//	@Override
		...
		if (PP_Product_BOM.isValidFromTo(getDateStartSchedule()))
		{
			...
			for (MPPProductBOMLine PP_Product_BOMline : PP_Product_BOM.getLines(true))
			{
				if (PP_Product_BOMline.isValidFromTo(getDateStartSchedule()))
				{
					...
					int wareHouseID = DB.getSQLValue(null,
							"SELECT m_warehouse_id FROM uy_productcategorywarehouse WHERE isdefault ='Y' AND m_product_category_id ="
									+ productLine.getM_Product_Category_ID());
					if (wareHouseID <= 0) {
						MProductCategory category = (MProductCategory) productLine.getM_Product_Category();
						throw new AdempiereException("La categoria de producto \"" + category.getName() + "\"\n no tiene un almacen por defecto.");
					}

					MPPOrderBOMLine PP_Order_BOMLine = new MPPOrderBOMLine(PP_Product_BOMline, getPP_Order_ID(), PP_Order_BOM.get_ID(), wareHouseID,
							get_TrxName());
			...
			MStorage[] storages, boolean forceIssue)
	{
		...
		for (MStorage storage : storages)
		{
			...
			if (qtyIssue.signum() != 0 || qtyScrap.signum() != 0 || qtyReject.signum() != 0)
			{
				...
				//Se acomoda codigo despues de importar clase pporderbomline
				String[] aux = new String[1];
				aux[0] = MPPOrderBOMLine.COMPONENTTYPE_Co_Product;
				// Method Variance
				...
				else if (PP_orderbomLine.isComponentType(aux))
				{
	...
	{
		//OpenUp M.R. 06-04-2011 Issue#583 Me cubro en caso de que alguno de los datos venga nulo
		if (getQtyDelivered() == null) return false;
		
		if (getQtyScrap() == null) return false;
		
		if (getQtyReject() == null) return false;
	...
	public void autoReportActivities()
	{
		...
		// Daba errores cuando el workflow de la orden es nulo.
		// Valido entonces que si es nulo no haga el for
		if (getMPPOrderWorkflow()==null) return; 
		// Fin OpenUp
	...
	/**
	 * OpenUp.
	 * Descripcion : Valida que este cargada la columna user en las BOMLines de la Orden.
	 * @param orderID
	 * @return String. Mensaje de validacion o NULL si esta todo bien.
	 * @author  Gabriel Vila 
	 * Fecha : 08/09/2010
	 */
	private String validateOrderUserBOMLine(int orderID){
		...
		try{
			sql ="SELECT coalesce(AD_User_ID,-1) as user " + 
			...
			boolean faltaUser = false;
			while (rs.next()){
				if (rs.getBigDecimal("user").intValueExact()<0)
					faltaUser = true;
				
			}
			
			if (faltaUser)
				value = "Falta indicar USUARIO en al menos una Linea de la Orden";
		}
		catch (Exception e)
		{
	...
	public  boolean isConfirmed(){
		...
		try{
			sql ="SELECT UY_ConfirmOrderHdr_ID  " + 
 		  		 " FROM UY_ConfirmOrderHdr " +
 		  		 " WHERE PP_Order_ID =?" +
 		  		 " AND DocStatus<>'VO'";
	 ...
	 * 
	 * OpenUp. Issue #752	
	 * Descripcion :Metodo que devuelve la cantidad disponible para facturar.
	 * @param idPPOrder
	 * @return
	 * @author  NS 
	 * Fecha : 29/06/2011
	 * @throws SQLException 
	 */
	public static BigDecimal getQtyAvailableToSale(int idPPOrder) {
		
		BigDecimal salida=BigDecimal.ZERO;
		
		ResultSet rs=null;
		PreparedStatement pstmt=null;
		String SQL=	"SELECT disponible FROM vuy_ordersalesid WHERE pp_order_id="+idPPOrder;	
		try{
			pstmt = DB.prepareStatement(SQL,null);
			rs = pstmt.executeQuery();
			
			if(rs.next()){
				salida=rs.getBigDecimal("disponible");
			}
		}catch (Exception e) {
	 ...
	 * 
	 * OpenUp.	
	 * Descripcion :Este metodo deja en correcto estado los campos UY_CerroConfirmacion y uy_incluidoenplan
	 * 				issue #762 Actualizado issue #871 06/09/2011
	 * @author  Nicolas Garcia 
	 * Fecha : 25/07/2011
	 */
	private void updatePlanClose() {
		
		if(this.isUY_CerroConfirmacion()){ //si la orden esta cerrada
			
			this.setuy_Incluidoenplan(false);//No tiene que estar en plan de produccion
		
		}else{//Si la orden esta abierta
			if(!this.isuy_Incluidoenplan()){//Si la orden no esta en el plan de produccion

				this.closeTechnical(null);
			}
		}
	 ...
	 * 
	 * OpenUp.	
	 * Descripcion :Metodo creado para el incidente Issue #811. Mejorado #824
	 	Lleva a cero la reserva para esa orden de venta, y reliza el cierre tecnico
	 * @return El metodo tandra que retornar null si todo salio ok, sino retorna mensaje de error.
	 * @author  Nicolas Garcia
	...
	public String closeTechnical(PO poDocSource) {
		...
		this.setuy_Incluidoenplan(false);
	 ...
	 * 
	 * OpenUp.	
	 * Descripcion :Issue #811 Metodo estatico que obtiene las Confirmaciones asociadas a una Orden de proceso
	 * El Mismo requiere que si se desea diferenciar solo para el caso de una orden que se coloque la condicion 
	 * en la clausula WHERE
	 * @return
	 * @author  Nicolas Garcia
	 * Fecha : 29/07/2011
	 */
	public static MConfirmorderhdr[] getAssociatedConfirmation(String whereClause, String trxName) {

		
		List<MConfirmorderhdr> lista = new Query(Env.getCtx(), MConfirmorderhdr.Table_Name, whereClause, trxName).list();

		return lista.toArray(new MConfirmorderhdr[lista.size()]);
	}
