...
public class JasperReportViewer extends JRViewer {
     ...
     * Metodo que devuelve el nombre que se usara para guardar en disco el reporte a partir del objeto jasperViewer
     */
    public String getTitulo(){
    	return this.jasperViewer.getTitulo();
    }
...
class ExportListener implements ActionListener {
    ...
    public void actionPerformed(ActionEvent event) {
        ...
        //OpenUp. Guillermo Brust. 14/06/2013. ISSUE #987
        //Se setea el escritorio como directorio por defecto en donde se abra el fileDialog
                       
        if(MSysConfig.getBooleanValue("UY_USER_DESKTOP_FILEDIALOG", false, Env.getAD_Client_ID(Env.getCtx()))){
        	
        	if(OpenUpUtils.isOSWindows()){
        		
        		String desktopPath = System.getProperty("user.home") + "\\"+"Desktop";    	
             	fileChooser.setCurrentDirectory(new File(desktopPath));
             	fileChooser.setSelectedFile(new File(viewer.getTitulo() + "." + viewer.getFormat().toLowerCase()));
        	}else{
