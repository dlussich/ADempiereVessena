...
public final class AppsAction extends AbstractAction
{
	...
	//	log.info( "AppsAction.actionPerformed", e.getActionCommand());
		...
		if (m_delegate != null){
			...
			// OpenUp. Gabriel Vila. 23/01/2014. Issue #1701
			// Ejecutaba dos veces la misma DocAction cuando es DocActionManual, y esto se debía a que se disparaba desde aca dos veces el actionperformed.
			// Modifique que si el delegate es del tipo ProcessModalDialog se setee a null para que no vuelva a dispararlo.
			if (m_delegate instanceof ProcessModalDialog){
				m_delegate = null;
			}
