	...
	implements FormPanel, ActionListener, ListSelectionListener
{
	...
	public int getActivitiesCount() 
	{
		...
		String sql = "SELECT count(*) FROM AD_WF_Activity a "
			...
			// Cuando los responsables estan parametrizados dinamicamente (se indica segun condicion determinada en ejecucion)
			// no puedo dejar que otro usuario tenga delegada la actividad que es para otro usuario. 
			// Esto no aplica cuando la responsabilidad es para un ROL y no para un unico usuario.
			// Comento y sustituyo.
			
			// + " WHERE a.AD_WF_Responsible_ID=r.AD_WF_Responsible_ID AND ur.AD_User_ID=?))";	//	#4
			+ " WHERE a.AD_WF_Responsible_ID=r.AD_WF_Responsible_ID AND ur.AD_User_ID=? AND r.responsibletype!='H'))";
	...
	public void display(IDColumn id)
	{
		...
		// Texgo de Descripcion y Ayuda dinámico segun PO
		if (m_activity.getPO() instanceof IDynamicWF){
			fDescription.setText (((IDynamicWF)m_activity.getPO()).getWFActivityDescription());
			fHelp.setText (((IDynamicWF)m_activity.getPO()).getWFActivityHelp());
		}
		// Fin OpenUp
	...
	private MWFActivity resetDisplay(IDColumn id)
	{
		...
		// OpenUp. Gabriel Vila. 07/12/2012. Issue #154.
		// Reset del Texto de Respuesta.
		fTextMsg.setText(null);
		// Fin OpenUp.
