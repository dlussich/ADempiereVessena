	...
	public void actionPerformed (ActionEvent e)
	{
		if (e.getActionCommand().equals(ConfirmPanel.A_OK))
		{
			...
			if (data != null && data.length() > 0)
			{
				...
				// OpenUp. Gabriel Vila. 18/06/2013. Issue #1034.
				// Si el PO de la tabla del chat implementa IDynamicTask, le envio 
				// el comentario.
				MTable table = (MTable)m_chat.getAD_Table();
				PO po = table.getPO(m_chat.getRecord_ID(), null);
				if (po instanceof IDynamicTask){
					((IDynamicTask)po).setComment(data);
				}
				// Fin OpenUp. Issue #1034
