...
public class AWindow extends CFrame
{
	...
	public boolean initWindow (int AD_Window_ID, MQuery query)
	{
		...
		if (loadedOK)
		{
			...
			// OpenUp. Gabriel Vila. 01/07/2013. Issue #1097
			// Si no tengo query, proceso posibilidad que la ventana este parametrizada
			// para entrar directamente en modalidad de Nuevo Registro.
			if (query == null){
				m_APanel.checkOpenOnNewRecord(AD_Window_ID);	
			}			
			// Fin OpenUp. Issue #1097.
		}
		return loadedOK;
	}	//	initWindow
...
//			+ " " + e);
	...
	public void dispose()
	{
		// OpenUp. Gabriel Vila. 15/02/2012. Issue #817.
		// Se agrega condicion y cambios en cierre de ventana por el issue
		// de necesitar confirmacion del usuario al salir.
		...
		if (m_APanel != null)
			//m_APanel.dispose();
			condicion = m_APanel.disposeOpen();
...
//			System.gc();
	...
	public void setWaitingMessage(String message){
		m_glassPane.setMessage(message);
	}
