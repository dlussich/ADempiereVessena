...
public class ProcessCtl implements Runnable
{
	...
	// OpenUp. Gabriel Vila. 05/03/2013. Issue #1701
	// DocActionManual
	private String docActionManualValue = null;
	// Fin OpenUp. Issue #1701
	...
	/***
	 * Para procesar el DocActionManual desde el cliente web necesito recibir el valor por parametro.
	 * OpenUp Ltda. Issue # 
	 * @author Gabriel Vila - 05/03/2014
	 * @see
	 * @param parent
	 * @param WindowNo
	 * @param parameter
	 * @param pi
	 * @param trx
	 * @param docActionValue : String con valor de docAction a ejecutar.
	 * @return
	 */
	public static ProcessCtl process(ASyncProcess parent, int WindowNo, IProcessParameter parameter, ProcessInfo pi, Trx trx, String docActionValue)
	{
		log.fine("WindowNo=" + WindowNo + " - " + pi);
		...
		{
			//synchrous
			worker.docActionManualValue = docActionValue;
			worker.run();
		}
		return worker;
	}	//	execute
	...
	//	try {System.out.println(">> sleeping ..");sleep(20000);System.out.println(".. sleeping <<");} catch (Exception e) {}
		...
		// OpenUp. Gabriel Vila. 06/12/2013. Issue #1701.
		boolean isDocActionManual = false;
		// Fin Issue #1701
		...
		try
		{
			...
			if (rs.next())
			{
				...
				// OpenUp. Gabriel Vila. 06/12/2013. Issue #1701.
				isDocActionManual = "Y".equalsIgnoreCase(rs.getString("IsDocActionManual"));
				// Fin Issue #1701.
		...
		// OpenUp. Gabriel Vila. 06/12/2013. Issue #1701.
		if (isDocActionManual){
			String mensaje = this.executeDocActionManual();

			if (mensaje != null){
				m_pi.setSummary(mensaje, true);
			}
	...
	/***
	 * Ejecucion manual de DocAction
	 * OpenUp Ltda. Issue #1701 
	 * @author Gabriel Vila - 06/12/2013
	 * @see
	 */
	private String executeDocActionManual(){
		...
		try
		{
			String docActionValue = null;

			if (this.m_parent != null){
				// Swing
				if (this.m_parent instanceof APanel){
					if (((APanel)this.m_parent).getCurrentTab() != null){
						if (((APanel)this.m_parent).getCurrentTab().getValue("DocAction") != null){
							docActionValue = ((APanel)this.m_parent).getCurrentTab().getValue("DocAction").toString();
						}
						
					}
				}		
			}
			// WebUI. Ya tiene el docaction 
			else if (this.docActionManualValue != null){
				docActionValue = this.docActionManualValue;
			} 

			// Si no obtuve valor de docaction manual no hago nada
			if (docActionValue == null){
				return null;
			}
			
			localTrx = Trx.get(Trx.createTrxName("DAM"), true);
			
			MTable table = MTable.get (Env.getCtx(), m_pi.getTable_ID());
			PO po = table.getPO(m_pi.getRecord_ID(), localTrx.getTrxName());
						
			// No dejo que entre dos veces a ejecutar la misma DocAction
			String docStatusActual = po.get_ValueAsString("DocStatus");
			if (docStatusActual != null){
				if (docStatusActual.equalsIgnoreCase(docActionValue)){
					return null;
				}
			}
			...
			if (po instanceof DocAction)
			{
				...
				try 
				{
					doc.processIt (docActionValue);
					mensaje = doc.getProcessMsg();
					doc.saveEx();
					//po.saveEx();
