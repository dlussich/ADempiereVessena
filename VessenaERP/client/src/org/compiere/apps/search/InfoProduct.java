...
public class InfoProduct extends Info implements ActionListener, ChangeListener
{
	...
	protected Info_Column[] getProductLayout()
	{
		...
		if (s_productLayout == null)
		{
			...
			// OpenUp. Gabriel Vila. 18/08/2011. Issue #781.
			// Cambio manera de obtener stock disponible.
			...
			list.add(new Info_Column(Msg.translate(Env.getCtx(), "QtyAvailable"), "case when p.IsBOM='N' and (p.ProductType!='I' OR p.IsStocked='N') then to_number(get_Sysconfig('QTY_TO_SHOW_FOR_SERVICES', '99999', p.ad_client_id, 0), '99999999999') else stk_available(p.M_Product_ID,?,null,null,null,null) end AS QtyAvailable", Double.class, true, true, null));
			// Fin Issue #781.
			...
			// OpenUp. Gabriel Vila. 18/08/2011. Issue #781.
			// Cambio manera de obtener cantidades de stock.
			...
			list.add(new Info_Column(Msg.translate(Env.getCtx(), "QtyOnHand"), "case when p.IsBOM='N' and (p.ProductType!='I' OR p.IsStocked='N') then to_number(get_Sysconfig('QTY_TO_SHOW_FOR_SERVICES', '99999', p.ad_client_id, 0), '99999999999') else stk_physical(p.M_Product_ID,?,null,null,null,null) end AS QtyOnHand", Double.class));
			list.add(new Info_Column(Msg.translate(Env.getCtx(), "QtyReserved"), "stk_reserved(p.M_Product_ID,?,null,null,null,null) AS QtyReserved", Double.class));
			list.add(new Info_Column("En Transito", "stk_ordered(p.M_Product_ID,?,null,null,null,null) AS QtyOrdered", Double.class));
			// Fin Issue #781.
	...
	private void initAtpTab (int  m_M_Warehouse_ID)
	{
		...
		sql += " w.Name, l.Value, w.M_Warehouse_ID, l.M_Locator_ID " // OpenUp. Gabriel Vila. 15/06/2011. Issue #719. Agrego ids de warehouse y locator a la select.
			+ "FROM M_Storage s"
		...
		if (!showDetail)
			sql += " GROUP BY productAttribute(s.M_AttributeSetInstance_ID), w.Name, l.Value, w.M_Warehouse_ID, l.M_Locator_ID ";  // OpenUp. Gabriel Vila. 15/06/2011. Issue #719. Agrego ids de warehouse y locator al group by.
		...
		try
		{
			...
			while (rs.next())
			{
				...
				// Informacion de Stock segun modelo de OpenUp. 
				BigDecimal qtyOnHand = MStockTransaction.getQtyPhysical(rs.getInt("m_warehouse_id"), rs.getInt("m_locator_id"), m_M_Product_ID, m_M_AttributeSetInstance_ID, null, null);
				BigDecimal qtyReserved = MStockTransaction.getQtyReserved(rs.getInt("m_warehouse_id"), rs.getInt("m_locator_id"), m_M_Product_ID, m_M_AttributeSetInstance_ID, null, null);
				BigDecimal qtyOrdered = MStockTransaction.getQtyOrdered(rs.getInt("m_warehouse_id"), rs.getInt("m_locator_id"), m_M_Product_ID, m_M_AttributeSetInstance_ID, null, null);
				// Fin OpenUp
				...
				line.add(qtyOnHand.doubleValue());       //  Qty
				line.add(null);						
				line.add(qtyOrdered.doubleValue());      //  QtyOrdered
				line.add(qtyReserved.doubleValue());     //  QtyReserved
