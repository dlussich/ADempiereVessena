	...
	implements ListSelectionListener
{
		...
		boolean multiSelection, String whereClause)
	{
		...
		if (tableName.equals("C_BPartner")){
			// OpenUp. Gabriel Vila. 13/04/2011. Issue #609
			// Ademas de IsSOtrx, verifico en el contexto si existe IsReceipt (para cuenta corriente).
			boolean showCustomers = true;
			if (!Env.getContext(Env.getCtx(),"IsSOTrx").equalsIgnoreCase("")) 
				showCustomers = (Env.getContext(Env.getCtx(),"IsSOTrx").equals("Y"));
			else if (!Env.getContext(Env.getCtx(),"IsReceipt").equalsIgnoreCase(""))
				showCustomers = (Env.getContext(Env.getCtx(),"IsReceipt").equals("Y"));
			
			info = new InfoBPartner (frame, modal, WindowNo,  value, showCustomers, multiSelection, whereClause);
			...
			// OpenUp. SBouissa 21/10/2015 Issue#4940
			if(MSysConfig.getBooleanValue("UY_RETAIL_INTERFACE", false, Env.getAD_Client_ID(Env.getCtx()))){
				info = new InfoMProductRT(frame, modal, WindowNo,  0,0, value,false, whereClause.replace("M_Product.", "p."));
			}else{
			...
			info = new InfoAssignment (frame, modal, WindowNo, value,
					multiSelection, whereClause);
		// OpenUp. Gabriel Vila. 17/03/2011. Issue #521.
		// Info window para buscar medios de pago desde una transaccion bancaria.
		else if (tableName.equals("UY_MediosPago"))
			info = new InfoMedioPago (frame, modal, WindowNo, value,
					multiSelection, whereClause);
		// Fin OpenUp.
