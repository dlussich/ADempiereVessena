	...
	implements ActionListener
{
	...
	public VLocationDialog (Frame frame, String title, MLocation location)
	{
		...
		// OpenUp. Nicolas Garcia. 24/08/2011. Issue #759.
		fdepartamento = new CComboBox(MDepartamentos.getDepartamentos(Env.getCtx(), m_location.getC_Country_ID()));
		fdepartamento.setSelectedItem((MDepartamentos) m_location.getUY_Departamentos());

		int departamento = m_location.getUY_Departamentos_ID();
		flocalidad = new CComboBox();
		if (departamento > 0) {

			fdepartamento.setSelectedItem((MDepartamentos) m_location.getUY_Departamentos());

			flocalidad = new CComboBox(MLocalidades.getLocalidades(Env.getCtx(), departamento));
			if (m_location.getUY_Localidades_ID() > 0) flocalidad.setSelectedItem((MLocalidades) m_location.getUY_Localidades());
			// ldepartamento.setText(m_location.getUY_Localidades().getName());
		}
		// Fin Issue #759
		...
		fdepartamento.addActionListener(this);
	...
	// private CLabel lAddress4 = new CLabel(Msg.getElement(Env.getCtx(),
	// "Address4"));
	...
	private CLabel				ldepartamento	= new CLabel("Departamento/Estado");
	private CComboBox			fdepartamento;

	private CLabel				llocalidad		= new CLabel("Localidad/Ciudad");
	private CComboBox			flocalidad;
	// Fin Issue #759
	...
	private void initLocation()
	{
		...
		// OpenUp. Nicolas Garcia. 24/08/2011. Issue #759.
		fdepartamento.removeAllItems();
		Object[] list = MDepartamentos.getDepartamentos(Env.getCtx(), m_location.getC_Country_ID());

		for (int i = 0; i < list.length; i++) {
			fdepartamento.addItem(list[i]);
		}
		...
		addLine(line++, ldepartamento, fdepartamento);
		addLine(line++, llocalidad, flocalidad);
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		} else if (e.getSource() == fdepartamento) {

			int departamento = 0;

			if (fdepartamento.getSelectedItem() != null) departamento = ((MDepartamentos) fdepartamento.getSelectedItem()).get_ID();

			m_location.setUY_Departamentos_ID(departamento);

			m_location.setUY_Localidades_ID(0);

			mainPanel.removeAll();

			initLocation();
			fdepartamento.requestFocus(); // allows to use Keybord selection
	...
	private void action_OK()
	{
		...
		if (fdepartamento.getSelectedItem() != null) m_location.setUY_Departamentos_ID(((MDepartamentos) fdepartamento.getSelectedItem()).get_ID());
		if (flocalidad.getSelectedItem() != null) m_location.setUY_Localidades_ID(((MLocalidades) flocalidad.getSelectedItem()).get_ID());
