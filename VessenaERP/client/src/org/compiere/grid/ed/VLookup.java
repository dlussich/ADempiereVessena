...
import org.compiere.apps.search.InfoMProductRT;
	...
	private void actionButton (String queryValue)
	{
		...
		else if (col.equals("M_Product_ID"))
		{
			...
			if(m_mField != null)
			{
				...
				// OpenUP FL 04/01/2010 M_PromotionGroupLine added to multiple selection 
				...
				multipleSelection = (MOrderLine.Table_ID ==  AD_Table_ID) || (MInvoiceLine.Table_ID == AD_Table_ID) || (I_PP_Product_BOMLine.Table_ID == AD_Table_ID) || (MProductPrice.Table_ID == AD_Table_ID) || (MPromotionGroupLine.Table_ID ==  AD_Table_ID);
				// Fin OpenUp
			}
			//	Show Info
			//INI OpenUp SBouissa 21/10/2015 Issue#4940
			Info i = null;
			if(MSysConfig.getBooleanValue("UY_RETAIL_INTERFACE", false, Env.getAD_Client_ID(Env.getCtx()))){
				i = new InfoMProductRT(frame, true, m_lookup.getWindowNo(),
						M_Warehouse_ID, M_PriceList_ID, queryValue, false, whereClause.replace("M_Product.", "p."));
				i.setVisible(true);
			}else{
				i = new InfoProduct (frame, true, m_lookup.getWindowNo(),
						M_Warehouse_ID, M_PriceList_ID, queryValue, multipleSelection, whereClause);
					i.setVisible(true);
			}
			cancelled = i.isCancelled();
			result = i.getSelectedKeys();
			//FIN OpenUp SBouissa 21/10/2015 Issue#4940
...
//			result = ip.getSelectedKeys();
		...
		else if (col.equals("C_BPartner_ID"))
		{
			...
			if (Env.getContext(Env.getCtx(), m_lookup.getWindowNo(), "IsSOTrx").equals("N"))
				isSOTrx = false;
			// OpenUp. Gabriel Vila. 13/04/2011
			// Ademas de IsSOtrx, verifico en el contexto si existe IsReceipt (para cuenta corriente).
			else if (Env.getContext(Env.getCtx(), m_lookup.getWindowNo(), "IsSOTrx").equals("Y")){
				isSOTrx = true;
			}
			else if (Env.getContext(Env.getCtx(), m_lookup.getWindowNo(), "IsSOTrx").equals("")){
				if (Env.getContext(Env.getCtx(), m_lookup.getWindowNo(), "IsReceipt").equals("N"))
					isSOTrx = false;
			}
			// Fin OpenUp
			
			
			// OpenUP FL 04/01/2010 multiple selection for client groups, UY_GruposClientesAsociados 
			if(m_mField != null) {
				int AD_Table_ID = MColumn.getTable_ID(Env.getCtx(), m_mField.getAD_Column_ID(), null);

				multipleSelection = (MGruposClientesAsociados.Table_ID==AD_Table_ID);
			}			
