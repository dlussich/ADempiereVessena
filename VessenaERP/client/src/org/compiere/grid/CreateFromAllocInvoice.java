 ...
 *
 */
public class CreateFromAllocInvoice extends CreateFrom {

	/**
	 * @param gridTab
	 */
	public CreateFromAllocInvoice(GridTab gridTab) {
		super(gridTab);
	}
	...
	public boolean save(IMiniTable miniTable, String trxName) {
		...
		BigDecimal totalAfectarPayments = Env.ZERO, divideRate = Env.ZERO;
		...
		if (hdr.getamtpayallocated().compareTo(Env.ZERO) > 0){
			tengoSaldoPayments = true;
			totalAfectarPayments = hdr.getamtpayallocated().subtract(hdr.getamtinvallocated());
		...
		for (int i = 0; i < miniTable.getRowCount(); i++) {
			...
			if (((Boolean) miniTable.getValueAt(i, 0)).booleanValue()) {
				...
				MAllocationInvoice line = new MAllocationInvoice(Env.getCtx(), 0, trxName);
				...
				// afectar en recibos (comparo siempre contra moneda 1 de afectacion).
	...
	protected Vector<Vector<Object>> getData(int adClientID, String isSOTrx, int cBPartnerID, Timestamp dateTrx,
		...
		String sql=	" SELECT al.c_invoice_id as recibo_id, al.c_doctype_id, doc.printname, al.documentno, " +
					...
					whereMonedas +
					" AND al.amtopen > 0" +
					" AND al.c_invoice_id NOT IN (SELECT coalesce(ali.c_invoice_id,0) FROM uy_allocationinvoice ali WHERE ali.uy_allocation_id=" + uyAllocationID + ")";
